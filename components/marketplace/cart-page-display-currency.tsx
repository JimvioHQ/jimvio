...
/** Cart header: reflects the userâ€™s display currency (converted from each orderâ€™s currency). */
export function CartPageDisplayCurrency() {
  ...
  return (
    <>
      ...
      <p className="text-xl font-black text-zinc-900 dark:text-white" title="Amounts are converted for display; checkout uses each orderâ€™s currency.">
        {loading ? "â€¦" : `${m.symbol} ${userCurrency}`}
      </p>
    </>
  );
}
