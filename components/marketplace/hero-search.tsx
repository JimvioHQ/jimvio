...
export function HeroSearch() {
  ...
  return (
    <div ref={containerRef} className="w-full z-[100] relative">
      {/* â”€â”€ Search bar "” matches HTML prototype search-wrap / search-bar â”€â”€ */}
      <div
      ...
      >
        ...
        >
          ...
          <button
            type="button"
            onClick={() => openAI()}
            className="flex items-center gap-1 px-[10px] py-[6px] rounded-none border-none bg-transparent text-[#a89f93] text-[10px] font-bold uppercase tracking-[.06em] hover:text-[#f97316] transition-colors shrink-0"
          >
          ...
          <button
            ...
            className="h-[38px] px-5 mr-1 rounded-none bg-[#1c1811] hover:bg-black dark:bg-[#f0e8dc] dark:text-[#1c1811] text-white text-[12px] font-bold tracking-[.04em] transition-colors shrink-0"
          >
            Explore
          </button>
        </div>
        ...
        <AnimatePresence>
          {q.length >= 2 && isFocused && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 4 }}
              className="absolute left-[52px] right-[52px] mt-2 bg-white dark:bg-[#1c1811] rounded-none border border-black/[.07] dark:border-white/[.08] shadow-[0_16px_48px_rgba(0,0,0,0.1)] overflow-hidden z-50"
            >
              <div className="p-3 space-y-1 max-h-[360px] overflow-y-auto no-scrollbar">
                  ...
                  <div className="flex flex-col gap-0.5">
                    {results.products.slice(0, 5).map((p) => (
                      <button
                        ...
                        className="flex items-center gap-3 p-3 rounded-none hover:bg-[#f5f4f1] dark:hover:bg-[#252219] text-left transition-all"
                      >
                ...
                <button
                  onClick={() => openAI()}
                  className="w-full h-11 rounded-none bg-[#fff5eb] dark:bg-[rgba(249,115,22,0.08)] text-[#c2410c] dark:text-[#fb923c] text-[11px] font-bold flex items-center justify-center gap-2 border border-[#fed7aa] dark:border-[rgba(249,115,22,0.18)] hover:bg-[#ffedd5] transition-all"
                >
