  ...
  return (
    <>
      ...
      {!chatOpen && (
        <div className="fixed bottom-20 right-4 z-50 md:hidden">
          <Button
            ...
            size="icon"
            className="h-14 w-14 rounded-sm shadow-none bg-[var(--color-accent)] hover:bg-[var(--color-accent-hover)] relative"
            aria-label="Chat with supplier"
          >
            <MessageCircle className="h-6 w-6" />
            {unreadCount > 0 && (
              <span className="absolute -top-0.5 -right-0.5 min-w-[20px] h-5 rounded-sm bg-red-500 text-white text-xs font-bold flex items-center justify-center">
      ...
      {showLoginPrompt && (
        ...
        >
          <div
            className="bg-[var(--color-surface)] rounded-sm shadow-none max-w-sm w-full p-6 text-center border border-[var(--color-border)]"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="w-14 h-14 rounded-sm bg-[var(--color-accent)]/10 flex items-center justify-center mx-auto mb-4">
            ...
            <div className="flex flex-col gap-3">
              <Button asChild className="w-full rounded-sm h-12 font-bold text-base">
                <Link href={loginUrl}>Sign in</Link>
              </Button>
              <Button variant="outline" className="w-full rounded-sm h-11" onClick={() => setShowLoginPrompt(false)}>
                Cancel
              </Button>
            </div>
          </div>
        ...
        <>
          ...
          <aside
            className={cn(
              ...
              "md:rounded-sm-2xl md:top-4 md:right-4 md:h-[calc(100vh-2rem)]"
            )}
          >
            {/* Header */}
            <div className="flex items-center gap-3 px-4 py-3 border-b border-[var(--color-border)] bg-[var(--color-surface)] shrink-0">
              <div className="h-11 w-11 rounded-sm bg-[var(--color-surface-secondary)] overflow-hidden shrink-0 border border-[var(--color-border)]/80 flex items-center justify-center">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-emerald-600 font-medium flex items-center gap-1.5 mt-0.5">
                  <span className="w-2 h-2 rounded-sm bg-emerald-500 ring-2 ring-emerald-500/30" />
                  Online
                </p>
              </div>
              <Button
                variant="ghost"
                size="icon"
                className="h-10 w-10 rounded-sm shrink-0 hover:bg-[var(--color-surface-secondary)]"
                onClick={() => setChatOpen(false)}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-0 bg-[var(--color-bg)]/50">
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  <div className="w-14 h-14 rounded-sm bg-[var(--color-surface-secondary)] flex items-center justify-center mb-3">
