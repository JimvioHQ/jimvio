  ...
  return (
    <div ref={containerRef} className={cn("relative w-full max-w-[600px] group isolate z-[100]", className)}>
      ...
      <Input
        ...
        autoComplete="off"
        className={cn(
          "w-full h-[42px] pl-12 pr-12 rounded-sm",
          "border border-border bg-white dark:bg-surface",
          "shadow-none",
          "text-[15px] font-semibold text-stone-900 dark:text-white",
          "placeholder:text-stone-400 placeholder:font-normal dark:placeholder:text-stone-500",
      ...
      {q.trim() && (
        ...
        >
          <div className="bg-zinc-100 dark:bg-surface-secondary p-1 rounded-sm group-hover/clear:bg-zinc-200 dark:group-hover/clear:bg-zinc-700 transition-colors">
      ...
      <AnimatePresence>
        {showDropdown && (q.trim().length > 0) && (
          <motion.div
            ...
            className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-surface border border-border rounded-sm shadow-none overflow-hidden"
          >
            <div className="p-2">
              {suggestions.length > 0 ? (
                <div className="space-y-1">
                  ...
                  {suggestions.map((p) => (
                    <Link
                      ...
                      className="flex items-center gap-3 px-3 py-2.5 rounded-sm hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors group/item mx-1"
                    >
                      <div className="h-10 w-10 rounded-sm overflow-hidden bg-stone-100 dark:bg-surface-secondary border border-border shrink-0">
                        {p.image ? (
                  ...
                  <Link
                    ...
                    onClick={() => setShowDropdown(false)}
                    className="group flex items-center justify-between w-[calc(100%-16px)] mx-2 my-2 py-3 px-5 text-[13px] font-bold text-white bg-stone-900 dark:bg-stone-800 hover:bg-black rounded-sm transition-all"
                  >
                ...
                <div className="px-4 py-8 text-center space-y-2">
                  <div className="inline-flex items-center justify-center w-12 h-12 rounded-sm bg-zinc-50 dark:bg-surface-secondary text-zinc-300 dark:text-zinc-600">
