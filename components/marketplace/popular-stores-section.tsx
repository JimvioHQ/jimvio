  ...
  return (
    <section className={cn("space-y-6 relative group/section", className)}>
      <div className="flex items-center justify-between px-1">
        <div>
          <h2 className="text-[24px] font-black text-zinc-900 flex items-center gap-3 tracking-tighter leading-none">
          ...
          <p className="text-[12px] text-zinc-500 font-bold mt-2 uppercase tracking-widest pl-1">
            Verified vendors · Follow for updates
          </p>
        </div>
        <Link
          href="/vendors"
          className="text-[11px] font-black text-zinc-400 hover:text-[#f97316] uppercase tracking-[0.2em] flex items-center gap-2 transition-all bg-white px-4 py-2 rounded-full border border-zinc-100 hover:border-orange-100 shadow-sm"
        >
        ...
        <div className="flex gap-5 overflow-x-auto no-scrollbar pb-6 pt-2 snap-x snap-mandatory">
          {stores.filter(s => (s.products?.length ?? 0) > 0).map((s) => {
            ...
            return (
              <div
                key={s.id}
                className="snap-start shrink-0 w-[300px] sm:w-[320px] group flex flex-col p-5 rounded-[32px] bg-white border border-zinc-100/80 shadow-sm hover:shadow-2xl hover:shadow-orange-500/10 hover:border-orange-500/30 transition-all duration-500 relative overflow-hidden"
              >
                {/* Background Decor */}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5 mb-1">
                        <h3 className="text-[15px] font-black text-zinc-900 truncate tracking-tight">
                          {s.business_name}
                        </h3>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider">{followers} followers</span>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {storeProducts.slice(0, 3).map((pr) => (
                      <Link
                        ...
                        className="group/product relative aspect-square rounded-2xl overflow-hidden bg-zinc-50 border border-zinc-100/50 hover:border-orange-500/40 transition-all shadow-sm"
                      >
                        {pr.images?.[0] ? (
                ...
                <div className="flex gap-2.5 mt-auto relative z-10">
                  ...
                  <Link href={storeUrl} className="flex-1">
                    <Button
                      className="w-full rounded-[14px] h-11 text-[11px] font-black bg-zinc-900 border-none text-white hover:bg-black hover:scale-[1.02] shadow-xl shadow-zinc-900/10 active:scale-95 transition-all"
                    >
                      Enter Store
                    </Button>
                  </Link>
          ...
          {/* See All Card */}
          <Link href="/vendors" className="snap-start shrink-0 w-[200px] flex flex-col items-center justify-center p-6 rounded-[32px] border-2 border-dashed border-zinc-200 hover:border-orange-500/40 hover:bg-orange-50/10 transition-all group">
             <div className="h-14 w-14 rounded-full bg-zinc-50 flex items-center justify-center group-hover:bg-orange-500 group-hover:scale-110 transition-all duration-300 mb-4 text-zinc-400 group-hover:text-white">
                <ChevronRight className="h-6 w-6" />
             </div>
             <p className="text-[13px] font-black text-zinc-400 group-hover:text-orange-600 transition-colors uppercase tracking-[0.2em] text-center">
