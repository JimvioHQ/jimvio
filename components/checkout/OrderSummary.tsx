...
export function OrderSummary({
  ...
  return (
    <div className="rounded-sm border border-[var(--color-border)] bg-[var(--color-surface)] p-5 sm:p-6 shadow-[var(--shadow-none)]">
      ...
      <ul className="space-y-4 mb-6 max-h-[min(360px,50vh)] overflow-y-auto pr-1 -mr-1 scrollbar-thin">
        {items.map((item) => (
          <li key={item.id} className="flex gap-3">
            <div className="h-14 w-14 shrink-0 overflow-hidden rounded-sm border border-[var(--color-border)] bg-[var(--color-surface-secondary)] flex items-center justify-center">
              {item.product_image ? (
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                {item.pricing_type === 'recurring' && (
                  <span className="text-[10px] font-bold text-orange-500 uppercase px-1.5 py-0.5 bg-orange-500/10 rounded-sm">
                    {item.billing_period}
                  </span>
                )}
              </div>
