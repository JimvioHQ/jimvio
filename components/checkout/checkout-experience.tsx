...
//    ChevronRight, CreditCard, CheckCircle2, Check, XCircle,
// } from "lucide-react";
...
//          if (!redirect) throw new Error("No redirect URL returned from payment provider");

//          // Store for fallback button
...
//             {/* ── Step 4: Order confirmed (full width, no sidebar) ── */}
//             {currentStep === 4 && (
//                <div className="max-w-lg mx-auto">
//                   <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl p-8 sm:p-10 text-center">

//                      {/* Icon */}
//                      <div className="mx-auto mb-6 w-16 h-16 rounded-2xl bg-orange-500/10 flex items-center justify-center">
...
//                         A payment tab has opened. Complete your payment there — your order will be confirmed automatically once payment is verified.
//                      </p>

//                      {/* Redirect fallback */}
//                      {redirectUrl && (
//                         <div className="mb-6 p-4 rounded-xl bg-[var(--color-surface-secondary)] border border-[var(--color-border)]">
//                            <p className="text-[12px] text-[var(--color-text-muted)] mb-2">
//                               Payment tab didn&apos;t open?
//                            </p>
//                            <button
//                               onClick={() => window.open(redirectUrl, "_blank")}
//                               className="text-[13px] font-semibold text-orange-500 hover:text-orange-400 transition-colors underline"
//                            >
...
//                      {/* Security note */}
//                      <div className="flex items-center gap-2.5 p-3 bg-[var(--color-surface-secondary)] rounded-xl border border-[var(--color-border)] mb-6">
//                         <ShieldCheck className="h-4 w-4 text-[var(--color-success)] flex-shrink-0" />
//                         <p className="text-[12px] text-[var(--color-text-muted)] text-left">
...
//                         {primaryOrderId && (
//                            <Button asChild className="flex-1 h-11 rounded-xl bg-orange-500 hover:bg-orange-600 text-white">
//                               <Link href={`/checkout/success?order=${primaryOrderId}`}>
...
export function CheckoutExperience({ orders, profile, mode = "cart" }: CheckoutExperienceProps) {
   ...
   // Sync payment method if the first order's currency changes
   useEffect(() => {
