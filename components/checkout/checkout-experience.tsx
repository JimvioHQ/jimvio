...
//  * `priceUSD` is the single source of truth; formatMoney("USD") converts to display currency.
//  */
...
// const STEPS: { n: 1 | 2 | 3 | 4; label: string; icon: React.ElementType }[] = [
...
//                   <span className="text-[10px] font-semibold text-blue-500 px-1.5 py-0.5 bg-blue-500/10 rounded-md uppercase">
//                      CJ
//                   </span>
//                )}
...
//    // ─── Pre-select saved CJ shipping method ──────────────────────────────────
...
//    const [cjShippingSelected, setCjShippingSelected] = useState<CJShippingOption | null>(() => {
...
//    const subtotal = useMemo(() => allItems.reduce((s, i) => s + Number(i.total_price), 0), [allItems]);

//    // CJ shipping is USD; convert to order currency before summing with subtotal.
//    // Guard against missing exchange rates to prevent NaN totals.
//    const shippingCostInOrderCurrency = useMemo(() => {
//       if (!cjShippingSelected) return 0;
//       const rate = rates[currency];
//       if (!rate || !Number.isFinite(rate)) return 0;
//       return cjShippingSelected.priceUSD * rate;
//    }, [cjShippingSelected, rates, currency]);

//    const total = subtotal + shippingCostInOrderCurrency;

//    const steps = useMemo(() => {
//       if (!isCJOrder) {
//          return STEPS.filter((s) => s.n !== 2).map((s, i) => ({ ...s, n: (i + 1) as 1 | 2 | 3 | 4 }));
//       }
//       return STEPS;
//    }, [isCJOrder]);

//    // ─── Fetch CJ shipping rates ───────────────────────────────────────────────

//    const fetchCJShippingRates = useCallback(async (countryCode: string): Promise<boolean> => {
...
//       try {
//          const res = await fetch("/api/cj/set-shipping", {
//             method: "POST",
...
//             // Reuse saved method when the shipping address hasn't changed
//             const addressUnchanged =
//                existingAddr?.country_code === countryCode &&
//                existingAddr?.city === shipping.city &&
//                existingAddr?.address1 === shipping.address1;

//             const hasSavedMethod =
//                !!savedShippingMethod && savedSupplierCost !== null;

//             const canSkipFetch =
//                hasSavedMethod && addressUnchanged && !!cjShippingSelected;
...
//       if (isCJOrder && currentStep === 2) {
//          if (!cjShippingSelected) return toast.error("Please select a shipping method");
//       }

//       const paymentStep = isCJOrder ? 3 : 2;
//       if (currentStep === paymentStep && !payment) return toast.error("Select a payment method");

//       setCurrentStep((s) => Math.min(s + 1, maxStep) as 1 | 2 | 3 | 4);
//       scrollTop();
//    }
...
//          setCurrentStep(isCJOrder ? 2 : 1);
//          return;
//       }

//       // FIX: Guard against NaN/zero total caused by missing exchange rates.
//       // This can happen if the rates context hasn't loaded yet or the currency
//       // key is absent from the rates map.
//       if (!Number.isFinite(total) || total <= 0) {
//          console.error("[handleComplete] Invalid total:", { total, subtotal, shippingCostInOrderCurrency, currency, rates });
...
//             shippingAmount: shippingCostInOrderCurrency,
...
//             if (!cjJson.success) throw new Error(cjJson.error ?? "Failed to save CJ shipping method");
//          }
...
//             });
//          } catch {
//             throw new Error("Could not reach the payment provider. Please check your connection and try again.");
//          }
...
//             data = await res.json();
//          } catch {
//             throw new Error(`Unexpected response from payment provider (HTTP ${res.status}). Please try again.`);
//          }

//          // FIX: Log the full API error response before throwing so the real
//          // cause (status code, error code, details) is visible in the console.
//          if (!res.ok) {
//             console.error("[handleComplete] Payment API error:", {
//                status: res.status,
//                endpoint: PAYMENT_ENDPOINTS[payment],
//                orderId: primaryOrderId,
//                currency,
//                total,
//                response: data,
...
//    const paymentStepN = isCJOrder ? 3 : 2;
//    const reviewStepN = isCJOrder ? 4 : 3;
...
//                      {isCJOrder && currentStep > 2 && cjShippingSelected && (
...
//                                     </div>
//                                     <button
//                                        onClick={() => setCurrentStep(paymentStepN as 1 | 2 | 3 | 4)}
//                                        className="text-[11px] font-semibold text-orange-500 hover:text-orange-400 transition-colors"
//                                     >
//                                        Change
//                                     </button>
...
//                                  )
//                               ) : (
//                                  <span className={isCJOrder ? "text-[var(--color-text-muted)]" : "text-[var(--color-success)] font-semibold"}>
...
//                      <button
//                         onClick={next}
//                         disabled={isCJOrder && currentStep === 1 && cjShippingLoading}
//                         className="flex items-center gap-2 h-11 px-6 rounded-xl bg-[var(--color-text-primary)] text-[var(--color-surface)] text-[13px] font-semibold transition-all hover:opacity-90 active:scale-[0.98] disabled:opacity-50"
//                      >
//                         {isCJOrder && currentStep === 1 && cjShippingLoading ? (
...
//                      onClick={next}
//                      disabled={isCJOrder && currentStep === 1 && cjShippingLoading}
//                      className="flex-1 flex items-center justify-center gap-2 h-11 rounded-xl bg-[var(--color-text-primary)] text-[var(--color-surface)] text-[13px] font-semibold transition-all hover:opacity-90 active:scale-[0.98] disabled:opacity-50"
//                   >
//                      {isCJOrder && currentStep === 1 && cjShippingLoading ? (
//                         <><Loader2 className="h-4 w-4 animate-spin" /> Loading rates...</>
//                      ) : (
