...
function tokens(isDark: boolean) {
    return {
        ...
        legendColor: isDark ? "rgba(255,255,255,0.45)" : "rgba(0,0,0,0.45)",
...
function GlobeCanvas() {
    ...
    // ── KEY FIX: don't read theme until mounted ──────────────────────────────
    const [mounted, setMounted] = useState(false);
    const { resolvedTheme } = useTheme();
    useEffect(() => { setMounted(true); }, []);

    // While not yet mounted, always treat as light so SSR output matches
    const isDark = mounted ? resolvedTheme === "dark" : false;
    const tk = tokens(isDark);

    // ── 1. Theme sync for canvas ref ─────────────────────────────────────────
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        function draw(rot: number) {
            ...
            LINKS.forEach(([a, b], linkIdx) => {
                ...
                ctx.fillStyle = nodeColor(na.type, dark); ctx.fill();
            });
            ...
            projected.sort((a, b) => a.pz - b.pz).forEach(n => {
                ...
                if (isHov) {
                    ...
                    ctx.globalAlpha = 1 - pulsePhase; ctx.stroke();
                    ctx.globalAlpha = 1;
                }
            });
            ...
            const logoSize = 28;
            const ping1 = (Date.now() % 2400) / 2400;
            const ping2 = ((Date.now() + 1200) % 2400) / 2400;
            [ping1, ping2].forEach(phase => {
            ...
            ctx.fillStyle = dark ? "rgba(253,80,0,0.14)" : "rgba(253,80,0,0.10)"; ctx.fill();
            ctx.strokeStyle = "rgba(253,80,0,0.55)"; ctx.lineWidth = 1.5; ctx.stroke();
            if (logoRef.current) {
                ctx.save();
    ...
    // Shared pill style — stable between SSR and first paint
    const pillStyle = {
        ...
        >
            ...
            >
                <span
                    ...
                    style={{ width: "6px", height: "6px", background: "#22c55e", display: "inline-block", flexShrink: 0 }}
                />
                <span suppressHydrationWarning style={{ fontSize: 10, color: tk.labelColor, transition: "color 0.35s ease" }}>
                    Live transactions:
                </span>
            ...
            {/* ── Legend ────────────────────────────────────────────────── */}
            <div className="absolute flex flex-col gap-2" style={{ top: "16px", right: "16px", pointerEvents: "none" }}>
                {[
                    { label: "Vendors", col: "#fd5000" },
                    { label: "Affiliates", col: "#f0a500" },
                    { label: "Buyers", col: tk.dotBuyer },
                ].map(l => (
                    <div key={l.label} className="flex items-center gap-2">
                        <div
                            suppressHydrationWarning
                            className="rounded-full"
                            style={{ width: "8px", height: "8px", background: l.col, flexShrink: 0 }}
                        />
                        <span
                            suppressHydrationWarning
                            style={{ fontSize: 10, color: tk.legendColor, fontWeight: 500, transition: "color 0.35s ease" }}
                        >
                            {l.label}
                        </span>
                    </div>
            ...
            <div
                ...
                style={{ bottom: "16px", left: "50%", transform: "translateX(-50%)", pointerEvents: "none", whiteSpace: "nowrap" }}
            >
                {[
                    ...
                    >
                        <div
                            ...
                            style={{ width: "6px", height: "6px", background: c.dot, flexShrink: 0 }}
                        />
                        <span suppressHydrationWarning style={{ fontSize: 9, color: tk.labelColor, transition: "color 0.35s ease" }}>
                            {c.label}
                        </span>
            ...
            {tooltip && (
                ...
                >
                    ...
                    <p style={{ fontSize: 10, color: tk.tooltipSub }}>{tooltip.sub}</p>
                </div>
            )}
        </div>
    );
