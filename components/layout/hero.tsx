...
// /* ─── Design tokens — all keyed off isDark ───────────────────────────────── */
...
//     // ─── isDarkRef is the single source of truth for the canvas loop.
//     //     isDark React state drives UI re-renders (badges, controls, etc).
//     //     They are kept in sync via the MediaQueryList listener below.
...
//     /* ── Dark mode sync ─────────────────────────────────────────────────────
//      * Both isDarkRef (canvas) and isDark state (React UI) are updated together.
//      * Never read isDark inside the canvas loop — always use isDarkRef.current.
//      */
//     useEffect(() => {
...
//             // Never allow zero/tiny dimensions — globe centre would be at (0,0)
//             if (w < 50) w = 520;
//             if (h < 50) h = 480;
...
//             // Scale once after resize — getContext returns the same context object
...
//         // proj always reads live cw/ch refs — never stale
...
//             // If sun projects to non-finite coords (behind globe), skip gradient
...
//             if (ulp.z < 0) return;
//             // Use isDarkRef — never the closed-over isDark state
...
//             // ─── Always read from ref inside loop — React state is stale here ───
//             const dark = isDarkRef.current;
//             const cw = cwRef.current, ch = chRef.current;
...
//     // ─── All UI tokens derived from isDark React state (not the ref) ─────
...
//                 style={{
//                     position: "relative",   // ← canvas anchors here
//                     overflow: "hidden",     // ← clips canvas + overlays to rounded rect
//                     isolation: "isolate",
...
//                         cursor: "grab", display: "block", touchAction: "none",
//                     }}
...
//                 {/* Stat chips */}
//                 <div
//                     className="absolute flex gap-2 flex-wrap justify-center pointer-events-none"
...
//                         <motion.div onClick={HandleLive} variants={fadeUp} className="flex justify-center lg:justify-start mb-7">
