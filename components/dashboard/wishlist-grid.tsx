  ...
  if (items.length === 0) {
    return (
      <GlassCard className="p-12 text-center border-dashed">
        ...
        <Button asChild className="mt-6 h-11 px-8 rounded-sm bg-stone-900 text-white font-bold text-[11px] uppercase tracking-widest shadow-none active:scale-95 transition-all hover:bg-stone-800">
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-6">
      {items.map(({ id: rowId, product }) => {
        ...
        return (
          <GlassCard key={rowId} className="overflow-hidden group hover:border-orange-200 transition-all p-1.5 bg-white dark:bg-surface/60 border-white shadow-none rounded-sm sm:rounded-sm">
            <Link href={`/marketplace/${product.slug}`} className="block relative">
              <div className="aspect-[4/3] rounded-sm sm:rounded-sm bg-white dark:bg-surface border border-stone-50 flex items-center justify-center p-3 relative overflow-hidden shadow-none">
                ...
                <button
                  ...
                  disabled={!!removingId}
                  className={cn(
                    "absolute top-2 right-2 p-2 rounded-sm shadow-none bg-white dark:bg-surface border border-stone-100 dark:border-border hover:bg-rose-50 hover:text-rose-600 text-rose-500 fill-rose-500 transition-all active:scale-90"
                  )}
