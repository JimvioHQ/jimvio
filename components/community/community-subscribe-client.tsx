...
export function CommunitySubscribeClient({
  ...
  return (
    <div className="min-h-screen bg-zinc-50/50 dark:bg-bg flex items-center justify-center py-6 md:py-12 px-0 sm:px-4">
      <div className="w-full max-w-5xl flex flex-col md:flex-row bg-white dark:bg-surface md:rounded-none overflow-hidden shadow-[0_32px_128px_-16px_rgba(0,0,0,0.12)] border border-zinc-100 dark:border-border">
        
        {/* ── SIDEBAR ── */}
        <aside className="w-full md:w-[320px] lg:w-[360px] flex-shrink-0 bg-[#1a1428] flex flex-col gap-8 p-8 relative overflow-hidden">
          <div className="absolute top-0 -right-20 w-64 h-64 bg-orange-500/10 rounded-none blur-[80px] pointer-events-none" />
          
          {/* Brand */}
          <div className="relative z-10 flex items-center gap-3">
            <div className="w-10 h-10 rounded-none bg-white dark:bg-surface/10 flex items-center justify-center border border-white/10">
          ...
          {/* Totals */}
          <div className="relative z-10 p-6 rounded-none bg-white dark:bg-surface/5 border border-white/10 ">
          ...
          <div className="relative z-10 flex-1 flex flex-col gap-6">
             <div className="flex gap-4 items-center">
                <div className="h-12 w-12 rounded-none border border-white/10 bg-white dark:bg-surface/5 overflow-hidden shrink-0 flex items-center justify-center font-black text-white text-lg">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="px-8 pt-8 pb-4">
            <div className="flex items-center gap-2 mb-6">
              {STEPS.map((step) => (
                <div key={step.n} className="flex-1 flex flex-col gap-2">
                  <div className={cn("h-1 rounded-none transition-all duration-500", step.n <= currentStep ? "bg-orange-500" : "bg-zinc-100 dark:bg-surface-secondary")} />
            ...
            <h2 className="text-2xl font-black text-[#1a1428] dark:text-white tracking-tight">
              ...
              >
                  ...
                  <div className="space-y-4">
                    {(
                      ...
                      <button
                        ...
                        onClick={() => setPlan(key)}
                        className={cn(
                          "w-full flex items-center justify-between p-6 rounded-none border-2 transition-all group",
                      ...
                      >
                         <div className="flex items-center gap-4">
                            <div className={cn("w-6 h-6 rounded-none border-2 flex items-center justify-center transition-all", plan === key ? "border-orange-500 bg-orange-500" : "border-zinc-200 dark:border-border-strong")}>
                         ...
                         <p className="text-lg font-black text-zinc-900 dark:text-white">{formatDisplayMoney(price, currency)}</p>
                      </button>
                    ))}
                  </div>
                )}
                ...
                {currentStep === 3 && (
                   <div className="space-y-6">
                      <div className="rounded-none bg-zinc-50 dark:bg-surface/50 border border-zinc-100 dark:border-border p-8 text-center space-y-4">
                      ...
                      <div className="p-5 rounded-none border border-zinc-100 dark:border-border bg-white dark:bg-surface shadow-none flex items-center justify-between">
                         <div>
          ...
          <div className="px-8 pb-8 pt-6 border-t border-zinc-50 dark:border-border/50 flex items-center justify-between gap-4">
            ...
            <button
              ...
              className={cn(
                "h-14 px-10 rounded-none bg-orange-500/10 border border-orange-500/30 active:scale-[0.98] disabled:opacity-50 text-orange-600 font-black text-sm transition-all shadow-none flex items-center gap-3 uppercase tracking-widest"
              )}
            >
              {submitting ? (
                <>
