...
export function SharedCommunityCard({ c, rank, showQuickActions = true }: CommunityCardProps) {
  ...
  return (
    <article className="group relative rounded-[32px] border border-stone-200/60 bg-white overflow-hidden shadow-sm hover:shadow-xl hover:border-orange-500/30 transition-all duration-500 hover:-translate-y-1 flex flex-col h-full">
      {/* Cover */}
      <div className="relative h-44 bg-stone-50 shrink-0">
        {c.cover_image ? (
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-orange-50/50 via-stone-50 to-white" />
        )}
      ...
      <div className="relative px-5">
        <div className="absolute -top-10 left-5 h-20 w-20 rounded-[28px] border-[4px] border-white bg-white overflow-hidden shadow-2xl z-10 transition-transform group-hover:scale-110 duration-500">
          {(c.avatar_url || c.image_url) ? (
            ...
            <div className="h-full w-full flex items-center justify-center text-orange-600 font-black text-2xl bg-orange-50">
              {c.name?.[0] ?? "?"}
            </div>
          )}
        </div>
      ...
      <div className="flex flex-col flex-1 p-5 pt-12 gap-3">
        <div>
          <h2 className="font-black text-stone-900 text-[18px] leading-tight line-clamp-1 group-hover:text-orange-600 transition-colors tracking-tight">{c.name}</h2>
          <p className="text-[12px] text-stone-500 mt-1 line-clamp-2 leading-relaxed font-medium">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {c.category && (
            <span className="text-[9px] font-black uppercase tracking-[0.15em] px-2 py-1 rounded-lg bg-stone-50 text-stone-400 border border-stone-100">
              {c.category}
            </span>
          )}
        ...
        <div className="mt-auto pt-4 border-t border-stone-100 space-y-3">
          <div className="flex items-center justify-between">
            {c.is_free ? (
              <span className="text-[11px] font-black text-orange-600 uppercase tracking-widest bg-orange-50 px-2 py-0.5 rounded-lg">
                Free Access
              </span>
            ) : c.monthly_price ? (
        ...
        {showQuickActions && (
          <div className="flex items-center gap-2 pt-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button
              className="flex-1 text-[10px] font-black uppercase tracking-wider text-stone-400 hover:text-orange-600 flex items-center justify-center gap-1.5 py-2 rounded-xl border border-transparent hover:border-orange-500/20 hover:bg-orange-50/50 transition-all"
              onClick={(e) => {
                e.preventDefault();
            ...
            <Link
              ...
              className="flex-1 text-[10px] font-black uppercase tracking-wider text-stone-400 hover:text-indigo-600 flex items-center justify-center gap-1.5 py-2 rounded-xl border border-transparent hover:border-indigo-500/20 hover:bg-indigo-50/50 transition-all"
