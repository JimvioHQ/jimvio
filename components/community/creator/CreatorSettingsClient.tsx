...
export function CreatorSettingsClient({ communityId }: { communityId: string }) {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      {error && (
        <div className="mb-4 p-3 rounded-xl border border-[var(--color-danger-light)] bg-[var(--color-danger-light)]/40 text-[var(--color-danger)] text-sm font-semibold">
          {error}
        </div>
      )}
      {successMsg && (
        <div className="mb-4 p-3 rounded-xl border border-[var(--color-success)]/20 bg-[var(--color-success)]/10 text-[var(--color-success)] text-sm font-semibold">
          {successMsg}
        </div>
      )}
      ...
      <section className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <Input value={slug} onChange={(e) => setSlug(e.target.value)} className="rounded-xl flex-1" />
          </div>
        </div>

        <div>
           <label className="text-xs font-bold text-[var(--color-text-muted)]">Tagline (short pitch)</label>
           <Input value={tagline} onChange={(e) => setTagline(e.target.value.substring(0, 100))} className="mt-1 rounded-xl" />
        ...
        <div>
           ...
           <Textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={4} className="mt-1 rounded-xl" />
        </div>
        ...
        <div>
          ...
          <select
            ...
            className="mt-1 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2 text-sm font-semibold outline-none"
          >
            {CATEGORIES.map((c) => (
      ...
      <section className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 space-y-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            <div className="h-20 w-20 shrink-0 rounded-full overflow-hidden border border-[var(--color-border)] bg-[var(--color-surface-secondary)]">
              {avatarUrl ? (
        ...
        <div>
          ...
          {coverUrl && (
            <div className="h-32 mb-3 rounded-2xl overflow-hidden border border-[var(--color-border)] relative max-w-md">
      ...
      <section className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 space-y-4">
        ...
        <div className="flex rounded-xl border border-[var(--color-border)] p-1 bg-[var(--color-surface-secondary)] max-w-xs">
          <button
            type="button"
            className={cn("flex-1 py-1.5 rounded-lg text-sm font-black transition-all", isFree ? "bg-[var(--color-surface)] shadow-sm text-[var(--color-text-primary)]" : "text-[var(--color-text-muted)]")}
          ...
          <div className="space-y-4 pt-2">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input value={monthly} onChange={(e) => setMonthly(e.target.value)} type="number" min="0" step="0.01" className="mt-1 rounded-xl" />
              </div>
              <div>
                <label className="text-xs font-bold text-[var(--color-text-muted)]">Yearly</label>
                <Input value={yearly} onChange={(e) => setYearly(e.target.value)} type="number" min="0" step="0.01" className="mt-1 rounded-xl" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-bold text-[var(--color-text-muted)]">Lifetime (optional)</label>
                <Input value={lifetime} onChange={(e) => setLifetime(e.target.value)} type="number" min="0" step="0.01" className="mt-1 rounded-xl" />
              </div>
              <div>
            ...
            <div>
              ...
              <Input value={trialDays} onChange={(e) => setTrialDays(e.target.value)} type="number" min="0" className="mt-1 rounded-xl max-w-40" />
            </div>
          </div>
        )}
      </section>
      ...
      <div className="flex justify-end gap-3 pt-2">
         <Button
            type="button"
            className="rounded-xl px-8 h-12 bg-[var(--color-accent)] hover:bg-[var(--color-accent-hover)] text-white font-black transition-all shadow-md active:scale-95"
            disabled={saving}
      ...
      <div className="mt-12 space-y-4 pt-10 border-t border-[var(--color-danger)]/20">
         <div className="rounded-2xl border border-[var(--color-danger)]/30 bg-[var(--color-danger-light)]/5 p-6 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div>
      ...
      <Dialog open={deleteModal} onOpenChange={setDeleteModal}>
         <DialogContent className="border-[var(--color-border)] bg-[var(--color-surface)]">
            ...
            <div className="space-y-4 py-4">
               ...
               />
               <Button
                 className="w-full rounded-xl mt-2 font-black"
                 variant="destructive"
