  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <>
          {tab === "Posts" && (
            <div className="rounded-2xl border border-[var(--color-border)] overflow-x-auto bg-[var(--color-surface)]">
          ...
          {tab === "Courses" && (
            <div className="space-y-3">
              ...
              {courses.length === 0 && <p className="text-sm text-[var(--color-text-muted)] py-8 text-center border border-dashed border-[var(--color-border)] rounded-2xl">No courses yet.</p>}
            </div>
          )}
          ...
          {tab === "Tasks" && (
            <div className="space-y-3">
              <div className="flex justify-end">
                <Button type="button" className="rounded-xl bg-[var(--color-accent)] hover:bg-[var(--color-accent-hover)] text-white font-black" onClick={() => setTaskModal(true)}>
                  Create Task
                </Button>
              </div>
              {tasks.map((t) => {
                ...
                return (
                  <div key={t.id} className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] overflow-hidden">
              ...
              {tasks.length === 0 && <p className="text-sm text-[var(--color-text-muted)] py-8 text-center border border-dashed border-[var(--color-border)] rounded-2xl">No tasks yet.</p>}
            </div>
          )}

          {tab === "Resources" && (
            <p className="text-sm text-[var(--color-text-muted)] py-8 text-center border border-dashed border-[var(--color-border)] rounded-2xl">
      ...
      <Dialog open={taskModal} onOpenChange={setTaskModal}>
        <DialogContent className="border-[var(--color-border)] bg-[var(--color-surface)] max-w-md">
          ...
          <div className="space-y-2">
            <Input placeholder="Title" value={tTitle} onChange={(e) => setTTitle(e.target.value)} className="rounded-xl" />
            <Textarea placeholder="Description" value={tDesc} onChange={(e) => setTDesc(e.target.value)} rows={3} className="rounded-xl" />
            <select value={tType} onChange={(e) => setTType(e.target.value)} className="w-full rounded-xl border border-[var(--color-border)] px-3 py-2 text-sm">
              <option value="daily">Daily</option>
              <option value="weekly">Weekly</option>
              <option value="challenge">Challenge</option>
              <option value="milestone">Milestone</option>
            </select>
            <select value={tDiff} onChange={(e) => setTDiff(e.target.value)} className="w-full rounded-xl border border-[var(--color-border)] px-3 py-2 text-sm">
              <option value="easy">Easy</option>
              <option value="medium">Medium</option>
              <option value="hard">Hard</option>
            </select>
            <Input type="number" value={tPoints} onChange={(e) => setTPoints(e.target.value)} className="rounded-xl" placeholder="Points" />
            <RoomSelect communityId={communityId} value={tRoom} onChange={setTRoom} filterTasks />
          </div>
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="outline" className="rounded-xl" onClick={() => setTaskModal(false)}>
              Cancel
            </Button>
            <Button className="rounded-xl bg-[var(--color-accent)] text-white font-black" disabled={saving} onClick={createTask}>
...
function RoomSelect({
  ...
  return (
    <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded-xl border border-[var(--color-border)] px-3 py-2 text-sm">
      {rooms.map((r) => (
