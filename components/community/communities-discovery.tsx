        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            {c.is_free ? (
              ...
              <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-white/10 text-white/70 border border-white/20">
                From {c.currency || "$"}{Number(c.monthly_price ?? 0).toFixed(0)}/mo
              </span>
            )}
          </div>
        </div>
        <div className="shrink-0 flex items-center gap-2 px-4 py-2 rounded-2xl bg-white text-[var(--color-accent)] font-black text-sm group-hover:bg-[var(--color-accent)] group-hover:text-white transition-all shadow-lg">
...
/* ─── Community Card ─── */
function CommunityCard({ c, rank }: { c: CommunityRow; rank?: number }) {
  const isNew = c.created_at && Date.now() - new Date(c.created_at).getTime() < 7 * 24 * 60 * 60 * 1000;

  return (
    <article className="group relative rounded-3xl border border-[var(--color-border)] bg-[var(--color-surface)] overflow-hidden shadow-sm hover:shadow-lg hover:border-[var(--color-accent)]/30 transition-all duration-300 hover:-translate-y-0.5 flex flex-col">
      {/* Cover */}
      ...
      <div className="relative px-4">
        <div className="absolute -top-7 left-4 h-14 w-14 rounded-2xl border-[3px] border-[var(--color-surface)] bg-[var(--color-surface-secondary)] overflow-hidden shadow-lg z-10">
          {c.avatar_url ? (
      ...
      {/* Body */}
      <div className="flex flex-col flex-1 px-4 pt-9 pb-4 gap-2">
        <div>
          <div className="flex items-center gap-1.5 flex-wrap">
            <h2 className="font-black text-[var(--color-text-primary)] text-base leading-tight line-clamp-1">{c.name}</h2>
          </div>
          <p className="text-xs text-[var(--color-text-muted)] mt-0.5 line-clamp-2 leading-relaxed">
            {c.tagline || "A community for like-minded people."}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {c.category && (
            <span className="text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full bg-[var(--color-surface-secondary)] text-[var(--color-text-muted)] border border-[var(--color-border)]">
              {c.category}
            </span>
          )}
        ...
        <div className="flex items-center justify-between gap-2 mt-auto pt-2 border-t border-[var(--color-border)]">
          {c.is_free ? (
            <span className="text-[11px] font-black px-2.5 py-1 rounded-lg bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
              Free to join
            </span>
          ) : c.monthly_price ? (
            <div>
              <span className="text-[11px] font-black text-[var(--color-accent)]">
                ...
                <span className="font-semibold text-[var(--color-text-muted)]">/mo</span>
              </span>
            </div>
          ) : (
            <span className="text-[11px] text-[var(--color-text-muted)]">Contact for pricing</span>
          )}

          <Button
            asChild
            size="sm"
            className="rounded-xl bg-[var(--color-accent)] hover:bg-[var(--color-accent-hover)] text-white font-black text-xs px-3 h-8 shadow-sm hover:shadow-md transition-all"
          >
            <Link href={`/communities/${c.slug}`}>
              {c.is_free ? "Join Free" : "View"} <ArrowRight size={13} className="ml-1" />
            </Link>
          </Button>
        </div>
        ...
        <div className="flex items-center gap-1.5 pt-1">
          <button
            ...
            onClick={(e) => {
              e.preventDefault();
              if (navigator.share) navigator.share({ title: c.name, url: `/communities/${c.slug}` });
              else navigator.clipboard.writeText(window.location.origin + `/communities/${c.slug}`);
            }}
          >
            <Globe size={11} /> Share
          </button>
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              {activeFilters.length > 0 && (
                <span className="h-5 w-5 rounded-full bg-white/20 flex items-center justify-center text-[10px] font-black">
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {(debounced ? items : rest).map((c, i) => (
                <CommunityCard key={c.id} c={c} rank={debounced ? i + 1 : i + 2} />
              ))}
            </div>
        ...
        {!loading && items.length > 0 && (
          <div className="mt-16 rounded-3xl bg-gradient-to-br from-[var(--color-accent)] to-[var(--color-accent-hover)] p-8 flex flex-col sm:flex-row items-center justify-between gap-6 text-white relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col sm:flex-row gap-3 shrink-0">
              <Button
                asChild
                className="rounded-2xl bg-white text-[var(--color-accent)] hover:bg-white/90 font-black shadow-xl hover:-translate-y-0.5 transition-all"
              >
                <Link href="/communities/create">
                  <Crown size={16} className="mr-2" /> Create for Free
                </Link>
              </Button>
              <Button
                asChild
                variant="outline"
                className="rounded-2xl border-white/30 text-white hover:bg-white/10 font-bold"
              >
