...
export function ChatRoom({ roomId, roomName, communityId, slug, hideHeader }: {
  ...
  return (
    <div className="wa-root flex flex-1 h-full min-h-0 overflow-hidden" style={{ fontFamily: 'inherit', height: '100%' }}>
      ...
      <div className="wa-panel">
        ...
        {isChatting && (
          <div className="md:hidden absolute left-3 top-2.5 z-[30]">
            <button type="button" className="flex h-10 w-10 items-center justify-center rounded-full bg-white/80 shadow-md" style={{ color: WA.accent }} onClick={() => setForceShowList(true)}>
              <ChevronLeft size={24} />
            </button>
          </div>
        )}
        ...
        {(activeConvId || roomId) && hideHeader && (
          <div className="absolute right-4 top-2.5 z-[30] flex items-center gap-2">
            <button onClick={() => startNativeCall('audio')} className="flex h-10 w-10 items-center justify-center rounded-full bg-white/90 shadow-md border border-gray-200 hover:scale-105 transition-all"><Phone className="h-4 w-4" style={{ color: WA.accent }} /></button>
            <button onClick={() => startNativeCall('video')} className="flex h-10 w-10 items-center justify-center rounded-full bg-white/90 shadow-md border border-gray-200 hover:scale-105 transition-all"><Video className="h-4 w-4" style={{ color: WA.accent }} /></button>
          </div>
        )}
        ...
        {!hideHeader && (
          <header className={`wa-hdr ${isChatting ? 'pl-14' : ''}`}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <p style={{ fontSize: 12, color: WA.accent }}>{activeConvId ? 'Online' : '● Live'}</p>
            </div>
              ...
              <div className="flex flex-col flex-1 min-w-0">
                {pendingMain.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 px-1 py-1.5 mb-1 rounded-lg" style={{ background: "rgba(0,0,0,0.04)" }}>
                    {pendingMain.map((a, i) => (
                      <div key={a.url} className="flex items-center gap-1.5 px-2 py-1 bg-white rounded-md shadow-sm text-[11px] font-medium" style={{ border: `1px solid ${WA.border}` }}>
      ...
      {/* ── Incoming call ── */}
      {incomingCall && (
        <div className="fixed inset-0 z-[30000] flex flex-col items-center justify-center bg-black/90 backdrop-blur-md text-white">
