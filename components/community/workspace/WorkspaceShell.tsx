...
import Link from "next/link";
import {
  Home, Radio, GraduationCap, Users, Trophy, Layers,
  Plus, ChevronDown, ChevronRight, Flame,
  MessageCircle, FileText, Folder, CheckSquare, LayoutList, Hash,
  MoreHorizontal, Star,
  type LucideIcon,
} from "lucide-react";
import { cn } from "@/lib/utils";
...
interface NavItem {
  href: string;        // relative to /c/community/[slug]
  label: string;
  icon: LucideIcon;
  matchExact?: boolean;
}
...
const ALLOWED_ROOM_PARAMS = new Set(["view", "space", "room"]);
...
function Avatar({ name, avatar, size = "md" }: { name: string; avatar?: string | null; size?: "xs" | "sm" | "md" | "lg" }) {
  ...
  return avatar ? (
    <div className={cn(dims, "rounded-full overflow-hidden ring-1 ring-border flex-shrink-0")}>
      <img src={avatar} alt={name} className="w-full h-full object-cover" />
    </div>
  ) : (
    <div className={cn(dims, "rounded-full flex items-center justify-center font-bold ring-1 ring-border flex-shrink-0", color)}>
      {(name?.[0] ?? "U").toUpperCase()}
    </div>
  );
}
...
export function WorkspaceLayout({
  ...
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const basePath = `/c/community/${community.slug}`;

  const urlView = searchParams.get("view");
  const [view, setView] = useState<WorkspaceView>(
    urlView === "admin" ? "admin" : initialView
  );
  const [expandedSpaces, setExpandedSpaces] = useState<Record<string, boolean>>(() =>
    Object.fromEntries(spacesWithRooms.map((s) => [s.id, true]))
  );
  ...
  const buildParams = (next: { view?: WorkspaceView; space?: string | null; room?: string | null }) => {
    const p = new URLSearchParams();
    for (const [k, v] of searchParams) {
      if (ALLOWED_ROOM_PARAMS.has(k)) p.set(k, v);
    }
    if (next.view !== undefined) {
      if (next.view === "admin") p.set("view", "admin");
      else p.delete("view");
    }
    if (next.space !== undefined) {
      if (next.space === null) p.delete("space");
      else p.set("space", next.space);
    }
    if (next.room !== undefined) {
      if (next.room === null) p.delete("room");
      else p.set("room", next.room);
    }
    return p;
  };
  ...
  const toggleView = () => {
    ...
    const next: WorkspaceView = view === "admin" ? "member" : "admin";
    setView(next);
    navigateWithParams(buildParams({ view: next }));
  };

  const openRoom = (spaceId: string, roomId: string) => {
    navigateWithParams(buildParams({ space: spaceId, room: roomId }));
  };

  const closeRoom = () => {
    navigateWithParams(buildParams({ space: null, room: null }));
  };
  ...
  return (
    <WorkspaceProvider value={ctx}>
      <CallProvider>
        <div className="flex flex-col bg-bg font-[family-name:var(--font-dm-sans)] h-full">
          <div
            className={cn(
              "px-4 lg:px-6 py-4 lg:py-6 pb-24 lg:pb-6",
              roomOverlayOpen
                ? "flex-1"
                : "lg:grid lg:grid-cols-[240px_1fr] lg:gap-5",
            )}
          >
            {/* ── Left sidebar ── */}
            {!roomOverlayOpen && (
            ...
            {/* ── Main content slot ── */}
            <main className={cn("relative min-w-0", roomOverlayOpen && "overflow-hidden")}>
              ...
              {roomOverlayOpen && (
                <WorkspaceRoomOverlay
                  communityName={community.name}
                  spaceName={activeSpace!.name}
                  spaceIconName={activeSpace!.icon ?? null}
                  roomName={activeRoom!.name}
                  roomType={activeRoom!.room_type}
                  onClose={closeRoom}
                >
          ...
          <MobileBottomNav basePath={basePath} pathname={pathname} />
        </div>
      </CallProvider>
    </WorkspaceProvider>
  );
...
function Sidebar({
  community, basePath, pathname, view, isAdmin, onViewToggle,
  ...
  const isPremium = false; // TODO: wire to community_memberships.plan_type !== 'free'

  const isActive = (item: NavItem) => {
    const fullPath = `${basePath}${item.href}`;
    if (item.matchExact) return pathname === fullPath;
    return pathname === fullPath || pathname.startsWith(`${fullPath}/`);
  };

  return (
    <div className="sticky top-0 flex flex-col h-[calc(100vh-var(--hub-topbar-h,56px))] overflow-y-auto pb-4">
      ...
      {isAdmin && (
        <div className="px-3 pb-2 flex-shrink-0">
          <button
            ...
            className={cn(
              "w-full px-2.5 py-1.5 text-[11px] font-semibold rounded-lg transition-all",
              view === "admin"
                ? "bg-[#fd5000]/10 text-[#fd5000]"
                : "bg-surface-secondary text-text-muted hover:text-text-primary",
            )}
          >
            Viewing as: {view === "admin" ? "Admin" : "Member"}
          </button>
        </div>
      )}
      ...
      <div className="flex-shrink-0 py-3">
        <div className="flex items-center justify-between px-3 mb-2">
          <p className="text-[10px] font-bold uppercase tracking-wider text-text-muted">Spaces</p>
          {isAdmin && view === "admin" && (
            <button
              type="button"
              aria-label="Create space"
        ...
        {isAdmin && view === "admin" && (
          <button
            type="button"
            className="w-full flex items-center gap-2.5 px-5 py-1.5 mt-1 rounded-xl text-[12px] font-medium text-text-muted hover:text-[#fd5000] hover:bg-[#fd5000]/5 transition-all"
          >
            <Plus className="w-3.5 h-3.5" />
            Create Space
          </button>
    ...
    <div className="p-3 rounded-xl bg-surface border border-border hover:border-border-hover transition-colors">
      <div className="flex items-center gap-2.5 mb-2.5">
        ...
        <button
          type="button"
          aria-label="More"
          className="w-5 h-5 flex items-center justify-center rounded-md hover:bg-surface-secondary text-text-muted transition-colors ml-auto"
        >
      ...
      <div>
        <div className="relative h-1.5 w-full overflow-hidden rounded-full bg-border">
          <div
            ...
            style={{
              width: `${pct}%`,
              background: close ? "linear-gradient(90deg,#fd5000,#f0b429)" : "#fd5000",
            }}
          />
        </div>
        <p className="mt-1 text-[10px] text-text-muted">
          {close ? (
            <span className="font-semibold text-[#fd5000]">
...
function MobileBottomNav({ basePath, pathname }: { basePath: string; pathname: string }) {
  return (
    <nav className="lg:hidden fixed bottom-0 inset-x-0 z-30 bg-bg/95 backdrop-blur-md border-t border-border safe-area-pb">
      <div className="grid grid-cols-5 h-14">
