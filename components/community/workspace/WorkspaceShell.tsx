...
//     router.replace(`/c/${community.slug}/workspace?${params.toString()}`, { scroll: false });
//   };
...
//                 className={cn(
//                   "min-w-0 relative",
//                   // Give it a contained surface so the overlay's rounded edges match
//                   "rounded-2xl",
//                   // Only force min-height when an overlay is mounted, so empty sections don't look broken
//                   activeRoom && "min-h-[calc(100vh-8rem)] bg-surface border border-border overflow-hidden"
//                 )}
//               >
//                 {/* Hide the section underneath while the overlay is open — no double work */}
//                 {!activeRoom && (isRoot ? sectionMap[section] : children)}
...
// }) {
//   return (
//     <header className="top-0  bg-bg backdrop-blur-md border-b border-border">
...
//             <ChevronRight className="text-text-primary w-4 h-4" />
//           </span>
//           <div className="flex items-center justify-center">
//             <span className="text-[13px] font-semibold text-text-primary capitalize text-accent/10 tracking-wider">
...
//           const peerIds = convData.map((conv: { user_high: string; user_low: string }) =>
...
//             (profilesData ?? []).map((p: { id: string, full_name: string | null, avatar_url: string | null, username: string | null }) => [p.id, p])
//           );

//           // Build conversations with profile info
//           const builtConversations = convData.map((conv: {
//             id: string; user_high: string; user_low: string; updated_at: string;
//             community_inbox_messages: { body: string; created_at: string; }[];
//           }) => {
          ...
          <div className={cn("mx-auto", isChatsView ? "max-w-none" : "max-w-[1400px]")}>
            <div
              className={cn(
                ...
                isChatsView
                  ? "px-0 lg:px-0"
                  : "px-4 lg:px-6 lg:grid lg:grid-cols-[268px_1fr] lg:gap-6"
              )}
            >
              {/* ── Left sidebar ─────────────────────────────────── */}
              {!isChatsView && (
              ...
              {/* ── Content wrapper (main + right rail) ──────────── */}
              {/* The overlay covers this whole wrapper so the sidebar stays visible */}
              <div className="relative min-w-0 min-h-[60vh]">
                <div
                  className={cn(
                    "min-w-0",
                    !isChatsView && "lg:grid lg:grid-cols-[1fr_320px] lg:gap-6"
                  )}
                >
                ...
                {/* ── Room overlay — covers main + right rail ───── */}
                {activeSpace && activeRoom && (
                  <WorkspaceRoomOverlay
                    communityName={community.name}
                    spaceName={activeSpace.name}
