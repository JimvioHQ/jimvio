...
function MediaCarousel({ media }: { media: UGCPostMedia[] }) {
  ...
  return (
    <div className="relative overflow-hidden bg-black/5 rounded-2xl">
      {current.type === "video" ? (
        <video
          src={current.url}
        ...
        <>
          ...
          {index > 0 && (
            <button
              ...
              className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 text-white flex items-center justify-center backdrop-blur-sm hover:bg-black/60 transition-colors"
            >
              ‹
            </button>
          )}
          {index < media.length - 1 && (
            <button
              onClick={() => setIndex((i) => i + 1)}
              className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 text-white flex items-center justify-center backdrop-blur-sm hover:bg-black/60 transition-colors"
            >
              ›
            </button>
          )}
...
function ProductStrip({ tags }: { tags: Array<{ products: UGCPostProduct }> }) {
  ...
  return (
    <div className="flex gap-2 flex-wrap mt-2">
      {tags.slice(0, 3).map(({ products: p }) => {
        ...
        return (
          <Link
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-[var(--color-surface-secondary)] border border-[var(--color-border)] hover:border-[var(--color-accent)]/40 hover:bg-[var(--color-accent-light)] transition-all group text-xs font-medium"
          >
            {img && (
              <Image src={img} alt={p.name} width={20} height={20} className="w-5 h-5 rounded-md object-cover" />
            )}
      ...
      {tags.length > 3 && (
        <span className="flex items-center px-3 py-1.5 rounded-xl bg-[var(--color-surface-secondary)] text-[var(--color-text-muted)] text-xs border border-[var(--color-border)]">
          +{tags.length - 3} more
        </span>
      )}
    </div>
...
export function UGCPostCard({ post, currentUserId, onDelete, className }: UGCPostCardProps) {
  ...
  return (
    <article
      className={cn(
        "bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl overflow-hidden transition-shadow hover:shadow-md",
        className
      )}
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 pt-4 pb-3">
        ...
        <div className="relative">
          <button
            ...
            className="p-2 rounded-xl text-[var(--color-text-muted)] hover:bg-[var(--color-surface-secondary)] hover:text-[var(--color-text-primary)] transition-colors"
          >
            <MoreHorizontal className="h-4 w-4" />
          </button>
          {showMenu && (
            <div className="absolute right-0 top-9 z-50 w-44 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-xl py-1 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
              {isOwner && (
                <button
      ...
      <div className="px-4 pb-4 flex items-center justify-between border-t border-[var(--color-border)]/60 pt-3">
        <div className="flex items-center gap-1">
          ...
          <Link
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium text-[var(--color-text-muted)] hover:bg-[var(--color-surface-secondary)] hover:text-[var(--color-text-primary)] transition-all"
          >
        ...
        <div className="flex items-center gap-1">
          {tags.length > 0 && (
            <Link
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold text-[var(--color-accent)] bg-[var(--color-accent-light)] hover:bg-[var(--color-accent)] hover:text-white transition-all"
            >
              <ShoppingBag className="h-3.5 w-3.5" />
              Shop now
            </Link>
          )}
          <button className="p-2 rounded-xl text-[var(--color-text-muted)] hover:bg-[var(--color-surface-secondary)] hover:text-[var(--color-text-primary)] transition-colors">
