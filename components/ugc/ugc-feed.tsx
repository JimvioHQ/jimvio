...
function SkeletonCard() {
  return (
    <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-sm overflow-hidden animate-pulse">
      <div className="flex items-center gap-3 px-4 pt-4 pb-3">
        <div className="w-9 h-9 rounded-sm bg-[var(--color-surface-secondary)]" />
        <div className="flex-1 space-y-1.5">
          <div className="h-3 bg-[var(--color-surface-secondary)] rounded-sm w-32" />
          <div className="h-2.5 bg-[var(--color-surface-secondary)] rounded-sm w-20" />
        </div>
      </div>
      <div className="mx-4 mb-3 aspect-[4/3] rounded-sm bg-[var(--color-surface-secondary)]" />
      <div className="px-4 space-y-2 mb-4">
        <div className="h-3 bg-[var(--color-surface-secondary)] rounded-sm w-full" />
        <div className="h-3 bg-[var(--color-surface-secondary)] rounded-sm w-3/4" />
      </div>
    </div>
  );
}
    ...
    <div className={cn("space-y-4", className)}>
      ...
      {showSortBar && (
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-1 p-1 rounded-sm bg-[var(--color-surface-secondary)] border border-[var(--color-border)]">
            {SORT_TABS.map((tab) => (
              <button
                key={tab.value}
                onClick={() => handleSortChange(tab.value)}
                className={cn(
                  "flex items-center gap-1.5 px-3.5 py-2 rounded-sm text-xs font-semibold transition-all",
                  sort === tab.value
      ...
      {!loading && posts.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="w-16 h-16 rounded-sm bg-[var(--color-surface-secondary)] flex items-center justify-center text-3xl mb-4">
            📸
          </div>
