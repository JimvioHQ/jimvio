 ...
 */
export function calculateCJPricing(
    costUsd: number,
    exchangeRate: number
): {
    price: number;
    ...
    // 1. Markup amount in USD
    const markupAmountUsd = costUsd * (tier.markupPercent / 100);

    // 2. Retail = cost + markup
    const retailUsd = costUsd + markupAmountUsd;

    // 3. Affiliate gets % of the markup amount
    const affiliateUsd = markupAmountUsd * (tier.commissionPercent / 100);

    // 4. Convert to RWF
    //    - price rounded UP to nearest 100 (clean shelf price)
    //    - affiliate rounded DOWN to nearest 10 (you never overpay)
    const price = Math.ceil((retailUsd * exchangeRate) / 100) * 100;
    const affiliate_price =
        Math.floor((affiliateUsd * exchangeRate) / 10) * 10;

    // 5. Strikethrough "original price" for marketing (30% above selling price)
...
export async function mapCJProductToJimvio(
    ...
    // ✅ Tier-based pricing (replaces old calculateRetailPrice)
    const {
        price,
        affiliate_price,
        ...
        markup_percent,
        compare_at_price,
    } = calculateCJPricing(costUsd, exchangeRate);
...
export function mapCJVariantToJimvio(
    ...
    const { price, affiliate_price, affiliate_commission_rate } =
        calculateCJPricing(priceUsd, exchangeRate);
