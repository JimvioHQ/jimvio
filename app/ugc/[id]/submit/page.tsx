...
const PLATFORMS = [
  { id: 'tiktok', name: 'TikTok', icon: Video, color: 'text-zinc-900 border-zinc-200 hover:border-zinc-900' },
...
export default function PublicUGCSubmitPage() {
  ...
  if (success) {
    return (
      <div className="min-h-[500px] flex flex-col items-center justify-center space-y-4 animate-fade-in">
        ...
        <h2 className="text-2xl font-black text-zinc-900">Successfully Submitted!</h2>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-fade-in pb-20 mt-10">
      <div className="flex items-center justify-between">
         <Link href={`/ugc/${id}`} className="group flex items-center gap-2 text-zinc-500 hover:text-zinc-900 font-black text-sm transition-all w-fit mb-4">
      ...
      <div className="bg-white rounded-[40px] border border-zinc-100 shadow-xl overflow-hidden">
         <div className="p-8 border-b border-zinc-100 bg-zinc-50">
            <h1 className="text-2xl font-black text-zinc-900">Submit Your Content</h1>
         ...
         <form onSubmit={handleSubmit} className="p-8 space-y-8">
            ...
            <div className="space-y-4">
               <label className="text-sm font-black text-zinc-900 uppercase tracking-widest">1. Select Platform</label>
                  ...
                  {PLATFORMS.map(p => (
                     <div 
                       ...
                       className={`flex flex-col items-center justify-center p-6 rounded-3xl border-2 cursor-pointer transition-all ${
                         platform === p.id 
                           ? `bg-zinc-50 border-orange-500 ring-4 ring-orange-100 shadow-md` 
                           : `${p.color} bg-white opacity-60 hover:opacity-100`
                       }`}
                     >
            ...
            <div className="space-y-4">
               <label className="text-sm font-black text-zinc-900 uppercase tracking-widest">2. Post URL</label>
               <input 
                 type="url" 
                 value={postUrl}
                 onChange={(e) => setPostUrl(e.target.value)}
                 placeholder="e.g. https://www.tiktok.com/@user/video/123456"
                 className="w-full h-14 bg-zinc-50 border border-zinc-200 rounded-2xl px-4 text-sm font-medium text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-all"
                 required
               />
            ...
            <div className="space-y-4">
               <div className="flex items-center justify-between">
                  <label className="text-sm font-black text-zinc-900 uppercase tracking-widest">3. Optional Note</label>
               </div>
               <textarea 
                 value={caption}
                 ...
                 className="w-full bg-zinc-50 border border-zinc-200 rounded-2xl px-4 py-4 text-sm font-medium text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-all resize-none"
               />
            </div>
