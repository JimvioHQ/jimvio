...
export default function JoinCampaignPage() {
  ...
  if (isJoined) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-50 p-6">
         <div className="max-w-md w-full bg-white rounded-[40px] p-10 border border-zinc-100 shadow-2xl text-center space-y-6">
            <div className="w-20 h-20 bg-emerald-100 rounded-full flex items-center justify-center mx-auto">
               <CheckCircle className="h-10 w-10 text-emerald-600" />
            </div>
            <div className="space-y-2">
               <h1 className="text-2xl font-black text-zinc-900">You're already in!</h1>
  ...
  return (
    <div className="min-h-screen bg-zinc-50/50 pb-20">
      {/* ── HEADER ── */}
      <div className="bg-white border-b border-zinc-100 sticky top-0 z-50 px-6 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
           <Link href={`/ugc/${id}`} className="flex items-center gap-2 text-zinc-500 hover:text-zinc-900 font-black text-sm transition-all">
      ...
      <div className="max-w-5xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-12 space-y-12">
           ...
           <div className="flex flex-col md:flex-row items-center gap-8 bg-zinc-950 rounded-[48px] p-8 md:p-12 text-white relative overflow-hidden shadow-2xl">
              ...
              <div className="relative shrink-0 w-32 h-32 md:w-40 md:h-40 rounded-[40px] bg-white p-1 overflow-hidden shadow-2xl">
              ...
              <div className="space-y-6">
                 ...
                 <div className="space-y-3">
                    {[
                       ...
                       <div key={i} className="flex items-center gap-4 p-5 rounded-[28px] bg-white border border-zinc-100 shadow-sm">
                          ...
                          <p className="text-sm font-black text-zinc-700">{r.label}</p>
                       </div>
                    ))}
                 </div>
              </div>
              ...
              <div className="space-y-6">
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    {campaign.media?.filter(m => m.usage === 'example').map((m, i) => (
                       <div key={i} className="aspect-[3/4] rounded-3xl overflow-hidden bg-zinc-100 border border-zinc-200 group relative">
           ...
           <section className="space-y-6">
              <h2 className="text-xl font-black text-zinc-900 tracking-tight flex items-center gap-2 text-center justify-center">
                 <Sparkles className="h-5 w-5 text-orange-500" /> The Road to Earnings
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                 {STEPS.map((s, i) => (
                    <div key={i} className="p-6 rounded-[32px] bg-white border border-zinc-100 text-center space-y-3 relative overflow-hidden group hover:border-orange-200 transition-all">
                       ...
                       <div className="w-12 h-12 rounded-2xl bg-zinc-50 border border-zinc-100 flex items-center justify-center mx-auto mb-2">
                          <s.icon className="h-5 w-5 text-zinc-900" />
                       </div>
                       <h3 className="font-black text-zinc-900 text-sm leading-tight uppercase tracking-wider">{s.title}</h3>
              ...
              <div className="max-w-md mx-auto space-y-6 text-center">
                 <label className="flex items-center gap-4 cursor-pointer group">
                    <div className={cn(
                       ...
                       agreed ? "bg-zinc-900 border-zinc-900" : "bg-white border-zinc-200 group-hover:border-zinc-400"
                    )}>
                    ...
                    <span className="text-left text-sm font-black text-zinc-700 select-none">
