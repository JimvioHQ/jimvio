      ...
      <div className="max-w-5xl mx-auto space-y-4 sm:space-y-8 px-4 sm:px-6 pt-4 sm:pt-10 relative z-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6">
           ...
           <div className="flex items-center gap-2">
              <Button asChild variant="outline" className="h-9 sm:h-10 px-6 rounded-xl bg-surface dark:bg-surface-secondary text-stone-900 dark:text-white border-border font-bold text-[9px] sm:text-[10px] uppercase tracking-widest active:scale-95 transition-all shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6">
           <div className="p-4 sm:p-5 rounded-2xl bg-surface/60 dark:bg-surface-secondary/40 border border-border shadow-sm flex items-center gap-4">
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-indigo-500/10 flex items-center justify-center text-indigo-500 shrink-0 border border-indigo-500/10">
           ...
           <div className="p-4 sm:p-5 rounded-2xl bg-surface/60 dark:bg-surface-secondary/40 border border-border shadow-sm flex items-center gap-4">
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-sky-500/10 flex items-center justify-center text-sky-500 shrink-0 border border-sky-500/10">
           ...
           <div className="p-4 sm:p-5 rounded-2xl bg-surface/60 dark:bg-surface-secondary/40 border border-border shadow-sm flex items-center gap-4">
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-emerald-500/10 flex items-center justify-center text-emerald-500 shrink-0 border border-emerald-500/10">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center">
           ...
           <div className="flex gap-1.5 overflow-x-auto no-scrollbar w-full md:w-auto pb-1 sm:pb-0">
              {["All", "Active", "Completed", "Cancelled"].map((f) => (
                 <button
                    key={f}
                    onClick={() => setFilter(f)}
                    className={cn(
                       "px-5 h-10 sm:h-11 rounded-lg text-[10px] font-bold uppercase tracking-widest whitespace-nowrap transition-all border shadow-sm shrink-0",
                       filter === f
                          ? "bg-stone-900 dark:bg-white text-white dark:text-stone-900"
                          : "bg-surface dark:bg-surface-secondary border-border text-stone-400 dark:text-text-muted hover:text-stone-900 dark:text-white"
                    )}
                 >
                    {f}
                 </button>
         ...
         <div className="space-y-3 sm:space-y-4">
            {filtered.length === 0 ? (
               <GlassCard className="py-20 text-center rounded-2xl border-dashed border-border bg-surface/20 dark:bg-surface-secondary/20">
                       ...
                       <GlassCard key={o.id} className="p-4 sm:p-6 rounded-2xl bg-surface/60 dark:bg-surface-secondary/40 border border-border hover:bg-surface dark:hover:bg-zinc-800 transition-all shadow-sm max-sm:-mx-4 max-sm:rounded-none max-sm:border-x-0">
                          <div className="flex flex-col lg:flex-row gap-6 sm:gap-8 items-start">
                             ...
                             <div className="flex-1 space-y-4 sm:space-y-6 w-full">
                                <div className="flex items-center justify-between">
                                   <div className="flex items-center gap-3">
                                      <div className={cn(
                                         "w-8 h-8 rounded-xl border flex items-center justify-center shadow-sm shrink-0",
                                ...
                                <div className="space-y-2.5">
                                   {o.order_items.map((item: any, idx: number) => (
                                      <div key={idx} className="flex items-center gap-3 sm:gap-4 group/item py-2 sm:py-3 border-t border-border/50 first:border-0 first:pt-0">
                                         <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-surface dark:bg-surface border border-border flex items-center justify-center shrink-0 font-black text-[9px] sm:text-[10px] text-stone-300 dark:text-stone-700 uppercase shadow-sm">
                                            {item.product_name[0]}
                                         </div>
                             ...
                             <div className="flex flex-row lg:flex-col gap-2 w-full lg:w-32 shrink-0">
                                <Button asChild className="flex-1 h-9 sm:h-10 rounded-xl bg-stone-900 dark:bg-white text-white dark:text-stone-900 font-bold text-[10px] uppercase tracking-widest shadow-sm border-none">
                                ...
                                <DropdownMenu>
                                   ...
                                   <DropdownMenuContent align="end" className="w-48 rounded-xl p-2 border-border shadow-sm bg-surface/95 dark:bg-surface/95 backdrop-blur-md">
                                      <DropdownMenuItem asChild className="rounded-lg focus:bg-surface-secondary dark:focus:bg-zinc-800 cursor-pointer">
                                      ...
                                      <DropdownMenuItem className="rounded-lg focus:bg-surface-secondary dark:focus:bg-zinc-800 cursor-pointer p-2.5 flex items-center gap-2.5">
                                      ...
                                      {o.status === "pending" && (
                                         <>
                                           ...
                                           <DropdownMenuItem onClick={() => handleCancelOrder(o.id)} className="rounded-lg focus:bg-rose-50 dark:focus:bg-rose-500/10 focus:text-rose-600 dark:focus:text-rose-400 text-rose-500 cursor-pointer p-2.5 flex items-center gap-2.5">
