...
import { MessageSquare, Heart, Pencil, Trash2, ExternalLink, Loader2 } from "lucide-react";
...
export function MyPostsList({ posts }: { posts: PostRow[] }) {
    ...
    <div className="grid gap-4 sm:grid-cols-1 lg:grid-cols-2">
      {list.map((post) => {
        ...
        return (
          <div
            key={post.id}
            className="bg-white/80 backdrop-blur-xl border border-[var(--color-border)] rounded-2xl p-5 flex flex-col"
          >
            {editingId === post.id ? (
