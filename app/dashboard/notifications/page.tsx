      ...
      <div className="min-h-screen flex flex-col items-center justify-center space-y-12 animate-in fade-in duration-700" style={{ background: "#f8f7f5" }}>
        <div className="relative">
          ...
          <div className="relative w-24 h-24 rounded-[32px] bg-white border border-white shadow-2xl flex items-center justify-center overflow-hidden">
            ...
            <Bell className="h-10 w-10 text-stone-900" />
          </div>
        </div>
        <div className="text-center space-y-3">
           <h2 className="text-[14px] font-black text-stone-900 uppercase tracking-[0.4em] pl-[0.4em]">Alert Hub</h2>
           <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest pl-[0.1em]">Syncing Your Notifications</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto space-y-12 px-6 pt-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
           ...
           <Button 
              variant="outline" 
              onClick={markAllRead} 
              className="h-14 px-8 rounded-full bg-white text-stone-900 border-white shadow-xl font-black text-[11px] uppercase tracking-widest active:scale-95 transition-all hover:bg-stone-50"
           >
              Mark All Read
           </Button>
        </div>
        ...
        <div className="space-y-6">
           {notifications.length === 0 ? (
              <GlassCard className="py-24 text-center rounded-[56px] border-dashed border-stone-200 bg-white/20">
                 ...
                 <h3 className="text-2xl font-black text-stone-900 tracking-tighter">Everything Clear</h3>
              ...
              <div className="space-y-4">
                 {notifications.map((n) => {
                    const cat = categoryIcons[n.type] || { icon: <Bell className="h-5 w-5" />, color: "text-stone-400", bg: "bg-stone-50" };
                    return (
                       <GlassCard key={n.id} className={cn(
                          ...
                          n.read ? "bg-white/40 border-white/60 hover:border-white hover:bg-white/60" : "bg-white shadow-2xl shadow-orange-500/5 ring-2 ring-orange-500/10"
                       )}>
                          <div className={cn(
                             ...
                             n.read ? "bg-white/60 text-stone-400" : cn("bg-white", cat.color)
                          )}>
                             {cat.icon}
                          </div>
                          ...
                          <div className="flex-1 min-w-0 space-y-2">
                             <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                   <p className="font-black text-xl tracking-tighter text-stone-900">{n.title}</p>
                                   {!n.read && (
                             ...
                             <div className="pt-4 flex items-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button className="text-[10px] font-black text-stone-900 uppercase tracking-widest flex items-center gap-2 hover:gap-3 transition-all">
        ...
        {/* Security Info */}
        <GlassCard className="p-10 rounded-[56px] border-white bg-stone-900 text-white relative overflow-hidden shadow-2xl">
           ...
           <div className="flex flex-col md:flex-row items-center justify-between gap-10">
              ...
              <Link href="/dashboard/settings?tab=security">
                 <Button className="h-16 px-10 rounded-3xl bg-white text-stone-900 font-black text-[11px] uppercase tracking-widest shadow-2xl active:scale-95 transition-all hover:bg-stone-50 shrink-0 border-none">
                    Security Settings
                 </Button>
              </Link>
           </div>
