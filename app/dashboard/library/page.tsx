        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 sm:gap-8">
           ...
           <div className="relative w-full md:w-80">
              ...
              <input
                 ...
                 className="w-full h-12 sm:h-14 pl-12 pr-6 rounded-2xl bg-surface dark:bg-surface border border-border text-[13px] font-bold text-stone-900 dark:text-white placeholder:text-stone-300 dark:placeholder:text-stone-600 shadow-xl focus:outline-none focus:bg-surface-secondary dark:focus:bg-zinc-800 transition-all"
              />
           </div>
        </div>

        {filtered.length === 0 ? (
           <GlassCard className="p-16 sm:p-24 text-center rounded-[40px] sm:rounded-[56px] border-border bg-surface dark:bg-surface/20">
              <div className="w-20 h-20 sm:w-24 sm:h-24 bg-surface dark:bg-surface rounded-[28px] sm:rounded-[32px] flex items-center justify-center mx-auto mb-6 sm:mb-8 border border-border shadow-xl">
              ...
              <Button asChild className="h-14 sm:h-16 px-10 sm:px-12 rounded-2xl sm:rounded-3xl bg-stone-900 dark:bg-white dark:bg-surface text-white dark:text-stone-900 dark:text-white font-black text-[10px] sm:text-[11px] uppercase tracking-widest shadow-2xl mt-8 sm:mt-10 hover:bg-black dark:hover:bg-stone-100 transition-all border-none">
           ...
           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8">
              {filtered.map((item) => {
                ...
                return (
                  <GlassCard key={item.id} className="overflow-hidden p-0 rounded-[32px] sm:rounded-[40px] border-border bg-surface dark:bg-surface/60 hover:shadow-2xl hover:bg-surface-secondary dark:hover:bg-zinc-800 transition-all duration-500 group">
                    {/* Thumbnail */}
                    <div className="aspect-[4/3] relative bg-stone-100 dark:bg-surface-secondary overflow-hidden">
                      ...
                      <div className="absolute inset-0 bg-stone-900/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3 backdrop-blur-[2px]">
                         {item.digital_download_url ? (
                           <a href={item.digital_download_url} target="_blank" rel="noopener noreferrer" download>
                             <Button size="icon" className="rounded-2xl h-12 w-12 sm:h-14 sm:w-14 bg-surface dark:bg-surface text-stone-900 dark:text-white hover:bg-surface-secondary dark:hover:bg-zinc-800 border-0 shadow-2xl hover:scale-110 transition-all">
                               <Download className="h-5 w-5 sm:h-6 sm:w-6" />
                             </Button>
                           </a>
                         ) : (
                           <Button size="icon" disabled className="rounded-2xl h-12 w-12 sm:h-14 sm:w-14 opacity-50 bg-surface dark:bg-surface border-0">
                      ...
                      {/* Type badge */}
                      <div className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-surface/90 dark:bg-surface/90 backdrop-blur-xl px-3 py-1.5 sm:px-4 rounded-xl text-[7px] sm:text-[8px] font-black uppercase tracking-[0.2em] text-stone-900 dark:text-white shadow-xl border border-border">
                          ⚡ Digital
                      </div>

                      {/* Access granted indicator */}
                      {item.digital_download_url && (
                        <div className="absolute bottom-3 left-3 sm:bottom-4 sm:left-4 bg-emerald-500/90 backdrop-blur-md px-2.5 py-1 rounded-lg text-[7px] sm:text-[8px] font-black uppercase tracking-widest text-white">
                          ✓ Access Granted
                        </div>
                      )}
                      {!item.digital_download_url && (
                        <div className="absolute bottom-3 left-3 sm:bottom-4 sm:left-4 bg-amber-500/90 backdrop-blur-md px-2.5 py-1 rounded-lg text-[7px] sm:text-[8px] font-black uppercase tracking-widest text-white">
                          ⏳ Processing
                        </div>
                      )}
                    </div>
                    ...
                    <div className="p-5 sm:p-8">
                      ...
                      <div className="flex items-center justify-between mb-4 sm:mb-5">
                        <div className="flex items-center gap-2 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-stone-400 dark:text-text-muted bg-surface-secondary dark:bg-surface-secondary px-2.5 py-1.5 rounded-lg">
                      ...
                      {item.digital_download_url ? (
                        <a href={item.digital_download_url} target="_blank" rel="noopener noreferrer" download className="block">
                          <Button className="w-full justify-center gap-2 sm:gap-3 rounded-xl sm:rounded-2xl text-[10px] sm:text-[11px] font-black uppercase tracking-widest h-12 sm:h-14 bg-stone-900 dark:bg-white dark:bg-surface text-white dark:text-stone-900 dark:text-white shadow-xl hover:bg-black dark:hover:bg-stone-100 active:scale-95 transition-all border-none">
                             <Download className="h-3.5 w-3.5 sm:h-4 sm:w-4" /> Download File
                          </Button>
                        </a>
                      ) : (
                        <Button disabled className="w-full justify-center gap-2 sm:gap-3 rounded-xl sm:rounded-2xl text-[10px] sm:text-[11px] font-black uppercase tracking-widest h-12 sm:h-14 bg-stone-200 dark:bg-surface-secondary text-stone-400 dark:text-text-muted border-none opacity-70">
