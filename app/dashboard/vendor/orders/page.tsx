                 ...
                 <tbody className="divide-y divide-stone-100/50">
                    {filtered.length === 0 ? (
                      ...
                      filtered.map((order) => {
                         ...
                         const productLabel = first ? (order.items.length > 1 ? `${first.product_name} +${order.items.length - 1}` : first.product_name) : "â€”";
                         ...
                         return (
                           <tr key={order.id} className="hover:bg-surface-secondary dark:hover:bg-zinc-800 transition-all duration-300 group">
                              ...
                              <td className="px-4 sm:px-8 py-5 sm:py-6">
                                 <p className="font-bold text-[13px] text-stone-900 dark:text-white leading-none">{order.buyer?.full_name?.split(' ')[0] || "â€”"}</p>
