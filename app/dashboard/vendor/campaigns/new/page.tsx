...
const PLATFORMS: { value: UGCPlatform; label: string; icon: string; border: string; bgHover: string }[] = [
  ...
  { value: 'youtube',   label: 'YouTube',   icon: '▶️', border: 'border-[#ff0000]/40', bgHover: 'hover:bg-[#ff0000]/5' },
  { value: 'x',         label: 'X/Twitter', icon: '✖️', border: 'border-zinc-400',     bgHover: 'hover:bg-zinc-100' },
];
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            {/* Core Blueprint */}
            <div className="rounded-2xl bg-[var(--color-surface-secondary)] border border-[var(--color-border)] p-6 shadow-sm">
               ...
               <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <textarea
                       ...
                       className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl px-4 py-3 text-sm text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] focus:outline-none focus:border-[var(--color-accent)] transition-all resize-none shadow-sm"
                    />
                  </div>
               </div>
            </div>
                  ...
                  <div className="space-y-2">
                     ...
                     <div className="flex items-center gap-2">
                        <div className="relative flex-1">
                           ...
                           <input type="text" value={hashtagInput} onChange={(e) => setHashtagInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addTag('required_hashtags', hashtagInput); setHashtagInput(''); } }} placeholder="viral, brand..." className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl pl-10 pr-4 py-2.5 text-sm focus:border-[var(--color-accent)] outline-none shadow-sm" />
                        </div>
                        <Button type="button" onClick={() => { addTag('required_hashtags', hashtagInput); setHashtagInput(''); }} className="py-2.5 px-6 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] text-sm font-semibold hover:bg-[var(--color-border)] transition-colors">Add</Button>
                     </div>
                     {form.required_hashtags.length > 0 && (
                        <div className="flex flex-wrap gap-2 pt-1">
                           {form.required_hashtags.map((h, i) => (
                              <span key={i} className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text-primary)] text-xs font-medium">
                  ...
                  <div className="space-y-2">
                     ...
                     <div className="flex items-center gap-2">
                        <div className="relative flex-1">
                           ...
                           <input type="text" value={mentionInput} onChange={(e) => setMentionInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addTag('required_mentions', mentionInput); setMentionInput(''); } }} placeholder="@yourbrand..." className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl pl-10 pr-4 py-2.5 text-sm focus:border-[var(--color-accent)] outline-none shadow-sm" />
                        </div>
                        <Button type="button" onClick={() => { addTag('required_mentions', mentionInput); setMentionInput(''); }} className="py-2.5 px-6 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] text-sm font-semibold hover:bg-[var(--color-border)] transition-colors">Add</Button>
                     </div>
                     {form.required_mentions.length > 0 && (
                        <div className="flex flex-wrap gap-2 pt-1">
                           {form.required_mentions.map((m, i) => (
                              <span key={i} className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)] text-blue-600 text-xs font-medium">
                  ...
                  <div className="space-y-2">
                     ...
                     <div className="flex items-center gap-2">
                        <div className="relative flex-1">
                           ...
                           <input type="text" value={keywordInput} onChange={(e) => setKeywordInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addTag('required_keywords', keywordInput); setKeywordInput(''); } }} placeholder="Must mention..." className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl pl-10 pr-4 py-2.5 text-sm focus:border-[var(--color-accent)] outline-none shadow-sm" />
                        </div>
                        <Button type="button" onClick={() => { addTag('required_keywords', keywordInput); setKeywordInput(''); }} className="py-2.5 px-6 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] text-sm font-semibold hover:bg-[var(--color-border)] transition-colors">Add</Button>
                     </div>
                     {form.required_keywords.length > 0 && (
                        <div className="flex flex-wrap gap-2 pt-1">
                           {form.required_keywords.map((k, i) => (
                              <span key={i} className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-zinc-800 text-white text-xs font-medium">
               ...
               <div className="space-y-8">
                  ...
                  <div className="flex flex-col sm:flex-row gap-6">
                     <div className="flex-1 space-y-3">
                        ...
                        <input type="url" value={thumbnailInput} onChange={(e) => setThumbnailInput(e.target.value)} placeholder="Paste image URL..." className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl px-4 py-2.5 text-sm shadow-sm focus:border-[var(--color-accent)] outline-none" />
                        <div className="flex gap-2">
                           ...
                           <div className="relative flex-1">
                              ...
                              <label htmlFor="thumbnail-upload" className={cn("flex items-center justify-center gap-2 w-full h-10 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] cursor-pointer hover:bg-zinc-50 transition-colors text-xs font-medium text-[var(--color-text-primary)] shadow-sm", uploading && "opacity-50 cursor-wait")}>
                                 {uploading ? <Loader2 className="h-3 w-3 animate-spin" /> : <Upload className="h-3 w-3" />} Upload
                              </label>
                           </div>
                        </div>
                     </div>
                     <div className="w-full sm:w-[160px] aspect-video rounded-xl bg-[var(--color-surface)] border border-[var(--color-border)] overflow-hidden relative flex flex-col items-center justify-center shadow-inner group shrink-0">
                        {form.media.find(m => m.usage === 'banner') ? (
                           <>
                              ...
                              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all">
                                 <button type="button" onClick={() => set('media', form.media.filter(m => m.usage !== 'banner'))} className="text-white text-xs font-semibold bg-red-600 px-3 py-1.5 rounded-lg hover:bg-red-700">Remove</button>
                              </div>
                           </>
                        ) : (
                           <>
                  ...
                  <div className="flex flex-col sm:flex-row gap-6">
                     <div className="flex-1 space-y-3">
                        ...
                        <input type="url" value={exampleInput} onChange={(e) => setExampleInput(e.target.value)} placeholder="Paste Reference Link..." className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl px-4 py-2.5 text-sm shadow-sm focus:border-[var(--color-accent)] outline-none" />
                        <div className="flex gap-2">
                           ...
                           <div className="relative flex-1">
                              ...
                              <label htmlFor="example-upload" className={cn("flex items-center justify-center gap-2 w-full h-10 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] cursor-pointer hover:bg-zinc-50 transition-colors text-xs font-medium text-[var(--color-text-primary)] shadow-sm", uploading && "opacity-50 cursor-wait")}>
                     ...
                     <div className="w-full sm:w-[160px] min-h-[100px] flex gap-2 flex-wrap items-start">
                        {form.media.filter(m => m.usage === 'example').map((m, i) => (
                           <div key={i} className="relative w-12 h-12 rounded-lg overflow-hidden border border-[var(--color-border)] shadow-sm group bg-zinc-100">
                              {m.type === 'image' ? (
                              ...
                              <button type="button" onClick={() => set('media', form.media.filter(x => x.url !== m.url))} className="absolute right-0.5 top-0.5 h-4 w-4 bg-black/60 text-white rounded-full flex items-center justify-center text-[8px] opacity-0 group-hover:opacity-100 hover:bg-red-500">✕</button>
                           </div>
                        ))}
                     </div>
                  </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="rounded-2xl bg-[var(--color-surface-secondary)] border border-[var(--color-border)] p-6 shadow-sm">
               ...
               <div className="grid grid-cols-1 gap-2">
                 {[
                   { value: 'clipping', label: 'Clipping', icon: '✂️', desc: 'Creators remix your content.' },
                   ...
                   <button
                     ...
                     className={cn(
                       ...
                       form.campaign_type === t.value
                         ...
                         : "bg-[var(--color-surface)] border-[var(--color-border)] hover:border-zinc-300 hover:bg-zinc-50"
                     )}
                   >
               ...
               {form.campaign_type === 'music_clipping' && (
                  <div className="mt-3 p-4 rounded-xl bg-orange-50 border border-orange-100 space-y-3 animate-fade-in">
                     <InputGroup label="Audio Track URL" value={form.music_track_url} setter={(v) => set('music_track_url', v)} placeholder="Spotify, Sound Link..." type="url" bg="bg-white" />
                     <InputGroup label="Artist Name" value={form.music_artist_name} setter={(v) => set('music_artist_name', v)} placeholder="e.g. Drake" type="text" bg="bg-white" />
                  </div>
               )}
               {form.campaign_type === 'promotion' && (
                  <div className="mt-3 p-4 rounded-xl bg-blue-50 border border-blue-100 space-y-3 animate-fade-in">
                     <InputGroup label="Promo Target Name" value={form.promotion_target} setter={(v) => set('promotion_target', v)} placeholder="e.g. 50% Off Course" type="text" bg="bg-white" />
                     <InputGroup label="Target URL" value={form.promotion_target_url} setter={(v) => set('promotion_target_url', v)} placeholder="Destination Link" type="url" bg="bg-white" />
                  </div>
               )}
            </div>
            ...
            <div className="rounded-2xl bg-zinc-950 p-6 text-white shadow-xl relative overflow-hidden">
               <div className="absolute top-0 right-0 w-32 h-32 bg-[var(--color-accent)] opacity-20 blur-3xl rounded-full translate-x-10 -translate-y-10" />
               ...
               <div className="relative z-10 space-y-5">
                  <div className="flex bg-white/10 p-1 rounded-xl">
                    <button type="button" onClick={() => set('payment_model', 'per_views')} className={cn("flex-1 py-2 text-[10px] font-bold uppercase tracking-wide rounded-lg transition-all", form.payment_model === 'per_views' ? "bg-white text-black shadow-sm" : "hover:text-white text-zinc-400")}>Per Views</button>
                    <button type="button" onClick={() => set('payment_model', 'fixed_per_content')} className={cn("flex-1 py-2 text-[10px] font-bold uppercase tracking-wide rounded-lg transition-all", form.payment_model === 'fixed_per_content' ? "bg-white text-black shadow-sm" : "hover:text-white text-zinc-400")}>Fixed Rate</button>
                  </div>

                  <div className="space-y-4">
                     {form.payment_model === 'per_views' ? (
                       <InputGroup label="Rate ($ USD / 1K Views)" value={form.rate_per_1k_views.toString()} setter={(v) => set('rate_per_1k_views', Number(v))} placeholder="3.00" type="number" step="0.1" align="font-semibold text-white focus:border-white" bg="bg-white/5 border-white/10" labelColor="text-zinc-400" />
                     ) : (
                       <InputGroup label="Fixed Rate Per Asset ($ USD)" value={form.fixed_rate.toString()} setter={(v) => set('fixed_rate', Number(v))} placeholder="150" type="number" step="1" align="font-semibold text-white focus:border-[var(--color-accent)]" bg="bg-white/5 border-white/10" labelColor="text-zinc-400" />
                     )}
                     <InputGroup label="Total Escrow Budget Limit ($ USD)" value={form.total_budget.toString()} setter={(v) => set('total_budget', Number(v))} placeholder="500" type="number" step="1" align="font-semibold text-white focus:border-white" bg="bg-white/5 border-white/10" labelColor="text-zinc-400" />
                  </div>
            ...
            {/* Targets & Deployment */}
            <div className="rounded-2xl bg-[var(--color-surface-secondary)] border border-[var(--color-border)] p-6 shadow-sm space-y-6">
               <div className="space-y-3">
                  <label className="block text-xs font-semibold text-[var(--color-text-muted)]">Allowed Platforms</label>
                  <div className="grid grid-cols-2 gap-2">
                     {PLATFORMS.map((p) => (
                        <button key={p.value} type="button" onClick={() => togglePlatform(p.value)} className={cn("p-2.5 rounded-xl transition-all border flex items-center justify-center gap-2", form.allowed_platforms.includes(p.value) ? `border-[var(--color-accent)] bg-violet-50/50 shadow-sm` : `border-[var(--color-border)] bg-[var(--color-surface)] ${p.bgHover}`)}>
                           <span>{p.icon}</span>
               ...
               <label className="flex items-center justify-between p-4 rounded-xl bg-[var(--color-surface)] border border-[var(--color-border)] cursor-pointer hover:bg-zinc-50 transition-colors shadow-sm">
                  ...
                  <div className={cn("w-10 h-6 rounded-full transition-all flex items-center px-1", form.requires_face ? "bg-[var(--color-accent)]" : "bg-zinc-200")}>
                     <div className={cn("h-4 w-4 bg-white rounded-full transition-transform shadow-sm", form.requires_face ? "translate-x-4" : "translate-x-0")} />
                  </div>
               </label>
            ...
            <div className="pt-2">
               {error && (
                 <div className="mb-4 p-3 rounded-xl bg-red-50 border border-red-100 text-xs font-medium text-red-600 text-center animate-fade-in shadow-sm">
                   {error}
                 </div>
               )}
               <Button type="submit" disabled={loading} className="w-full h-[52px] rounded-xl bg-gradient-to-r from-[var(--color-accent)] to-[var(--color-accent-hover)] text-white font-semibold text-sm shadow-md hover:shadow-lg transition-all active:scale-[0.98]">
   ...
   return (
      <div className="space-y-1.5 w-full">
         ...
         <input 
           ...
           className={cn("w-full border border-[var(--color-border)] rounded-xl px-4 py-2.5 text-sm text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] focus:outline-none focus:border-[var(--color-accent)] transition-colors shadow-sm", bg, align)} 
         />
      </div>
   );
}
