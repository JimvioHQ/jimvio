  ...
  if (!vendor) {
    return (
      <div className="min-h-screen flex items-center justify-center px-6" style={{ background: "var(--color-bg)" }}>
        <GlassCard className="max-w-md w-full p-8 text-center rounded-none border-border shadow-none bg-surface dark:bg-surface/60">
          <div className="w-20 h-20 bg-surface dark:bg-surface rounded-none flex items-center justify-center mx-auto mb-8 border border-border shadow-none">
          ...
          <Button asChild className="w-full h-12 rounded-none bg-orange-500 text-white hover:bg-orange-600 font-bold active:scale-95 transition-all text-sm shadow-none border-none">
    ...
    >
      <div className="max-w-6xl mx-auto space-y-8 px-6 pt-10 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
           <div className="flex flex-wrap items-center gap-2 p-1.5 rounded-none bg-surface dark:bg-surface border border-border shadow-none">
              ...
              {allVendors.map((v) => (
                 <button
                    ...
                    className={cn(
                       "px-5 py-2 rounded-none text-[10px] font-bold uppercase tracking-widest transition-all",
                       vendor?.id === v.id 
                          ? "bg-stone-900 text-white shadow-none scale-105" 
           ...
           <Button className="h-11 px-6 rounded-none bg-surface dark:bg-surface text-stone-900 dark:text-white font-bold text-[11px] uppercase tracking-widest shadow-none active:scale-95 transition-all hover:bg-surface-secondary dark:bg-surface/50 border border-border" asChild>
        ...
        <div className="relative group">
           <div className="h-64 sm:h-80 rounded-none overflow-hidden bg-stone-100 border border-white shadow-none relative">
              ...
              {editing ? (
                 <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/40 backdrop-blur-3xl">
                    <CloudinaryUploadButton
                       ...
                       className="bg-white dark:bg-surface text-stone-900 dark:text-white h-11 px-8 rounded-none font-bold text-[11px] uppercase tracking-widest shadow-none active:scale-95 transition-all border-none"
                    />
                 </div>
              ) : (
              ...
              <div className="absolute bottom-6 left-6 right-6 z-20 flex flex-col sm:flex-row sm:items-end justify-between gap-6">
                 <div className="flex items-center gap-6">
                    <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-none bg-surface dark:bg-surface border-[3px] border-border shadow-none relative overflow-hidden shrink-0">
                       {editing ? (
                          <div className="absolute inset-0 flex items-center justify-center bg-surface dark:bg-surface z-20">
                             <CloudinaryUploadButton
                                ...
                                buttonText="LOGO"
                                className="text-[10px] font-bold uppercase bg-stone-50 dark:bg-surface/50 text-stone-900 dark:text-white h-full w-full rounded-none border-none"
                             />
                          </div>
                       ) : (
                    ...
                    <div className="space-y-1">
                       <div className="flex items-center gap-2">
                          <Badge variant="secondary" className="bg-orange-500 hover:bg-orange-500 text-white border-none text-[8px] font-bold px-2 py-0.5 rounded-none uppercase tracking-widest">Verified Vendor</Badge>
                       </div>
                       {editing ? (
                          <Input 
                             value={form.business_name} 
                             onChange={(e) => setForm(f => ({ ...f, business_name: e.target.value }))}
                             className="h-12 bg-white dark:bg-surface/20 backdrop-blur-xl border-white/20 text-white text-2xl font-bold rounded-none mb-1 w-full max-w-sm placeholder:text-white/40 focus:ring-0"
                          />
                       ) : (
                 ...
                 <div className="flex items-center gap-2">
                    {editing ? (
                       <>
                          ...
                          <Button disabled={saving} onClick={handleSave} className="h-10 px-6 rounded-none bg-white dark:bg-surface text-stone-900 dark:text-white hover:bg-white dark:bg-surface/90 font-bold text-[10px] uppercase tracking-widest shadow-none active:scale-95 transition-all border-none">
                       ...
                       <>
                          <Link href={`/vendors/${vendor.business_slug}`} target="_blank">
                             <Button variant="outline" className="h-10 px-5 rounded-none border-white/20 bg-white dark:bg-surface/10 backdrop-blur-xl text-white hover:bg-white dark:bg-surface/20 font-bold text-[10px] uppercase tracking-widest active:scale-95 transition-all">
                                View Public <ExternalLink className="h-3 w-3 ml-2" />
                             </Button>
                          </Link>
                          <Button onClick={() => setEditing(true)} className="h-10 px-6 rounded-none bg-orange-500 text-white hover:bg-orange-600 font-bold text-[10px] uppercase tracking-widest shadow-none active:scale-95 transition-all border-none">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
           ...
           <div className="lg:col-span-8 space-y-8">
              ...
              {/* Description Box */}
              <GlassCard className="p-8 rounded-none bg-surface dark:bg-surface/60 border-border shadow-none relative overflow-hidden">
                 ...
                 {editing ? (
                    <Textarea 
                       ...
                       className="min-h-[140px] rounded-none bg-white dark:bg-surface border-stone-100 dark:border-border text-sm font-medium leading-relaxed shadow-none focus:ring-4 focus:ring-stone-500/5 focus:border-stone-200 dark:border-border p-6 resize-none"
              ...
              <section className="space-y-6">
                 <div className="flex items-center justify-between px-1">
                    ...
                    <Link href="/dashboard/products/new">
                       <Button className="h-10 px-5 rounded-none bg-stone-900 text-white font-bold text-[10px] uppercase tracking-widest shadow-none hover:bg-black transition-all border-none">
                 ...
                 {products.length === 0 ? (
                    <GlassCard className="py-16 text-center rounded-none border-dashed border-border bg-surface dark:bg-surface/20">
                       ...
                       <Link href="/dashboard/products/new">
                          <Button className="h-11 px-8 rounded-none font-bold text-[10px] uppercase tracking-widest shadow-none bg-white dark:bg-surface border-stone-100 dark:border-border text-stone-900 dark:text-white hover:bg-stone-50 dark:bg-surface/50">
                             Add Product
                          </Button>
                       </Link>
                    </GlassCard>
                     ...
                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                       {products.map((p) => (
                          <Link key={p.id} href={`/dashboard/products/${p.id}/edit`} className="group outline-none">
                             <div className="flex items-center gap-4 p-4 rounded-none bg-surface dark:bg-surface border border-border shadow-none hover:bg-surface-secondary dark:hover:bg-zinc-800 hover:shadow-none transition-all active:scale-[0.98]">
                                <div className="w-16 h-16 rounded-none bg-surface-secondary dark:bg-surface-secondary border border-border shadow-none overflow-hidden p-0.5 shrink-0">
                                   {p.images?.[0] ? (
                                      <img src={p.images[0]} alt="" className="w-full h-full object-cover rounded-none group-hover:scale-105 transition-transform" />
                                   ) : (
                                      <div className="w-full h-full bg-surface-secondary dark:bg-surface/50 flex items-center justify-center rounded-none"><Package className="h-5 w-5 text-stone-400" /></div>
                                   )}
                                </div>
                                ...
                                <div className="flex-1 min-w-0">
                                   <div className="flex items-center gap-2 mb-0.5">
                                      <span className={cn(
                                        "text-[8px] font-bold px-1.5 py-0.5 rounded-none uppercase tracking-widest",
                                ...
                                <div className="h-8 w-8 rounded-none bg-stone-50 dark:bg-surface/50 flex items-center justify-center text-stone-300 group-hover:bg-stone-900 group-hover:text-white transition-all shadow-none">
           ...
           <div className="lg:col-span-4 space-y-6">
              ...
              <div className="grid grid-cols-1 gap-4">
                 {[
                    ...
                    <div key={i} className="flex items-center justify-between p-6 rounded-none bg-surface dark:bg-surface border border-border shadow-none">
                       <div className="flex items-center gap-4">
                          <div className={cn("w-10 h-10 rounded-none flex items-center justify-center shrink-0 border border-border bg-surface-secondary dark:bg-surface-secondary shadow-none", stat.color)}>
              ...
              {/* Promo Banner - Softer */}
              <GlassCard className="p-8 rounded-none bg-stone-900 text-white relative overflow-hidden shadow-none border-none">
                 <div className="absolute top-0 right-0 w-32 h-32 bg-orange-500/20 blur-[60px] rounded-none translate-x-1/2 -translate-y-1/2" />
                 ...
                 <Link href="/dashboard/vendor/campaigns/new">
                    <Button className="w-full bg-white dark:bg-surface text-stone-900 dark:text-white hover:bg-stone-50 dark:bg-surface/50 h-11 rounded-none font-bold text-[10px] uppercase tracking-widest active:scale-95 transition-all outline-none border-none">
              ...
              {/* Public Link Card */}
              <div className="p-8 rounded-none bg-surface dark:bg-surface border border-border shadow-none text-center space-y-4">
                 <div className="w-10 h-10 bg-surface-secondary dark:bg-surface-secondary rounded-none flex items-center justify-center mx-auto text-stone-500">
