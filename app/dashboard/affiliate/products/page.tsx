      ...
      <div className="min-h-screen flex flex-col items-center justify-center space-y-12 animate-in fade-in duration-700" style={{ background: "#f8f7f5" }}>
        <div className="relative">
          ...
          <div className="relative w-24 h-24 rounded-[32px] bg-white border border-white shadow-2xl flex items-center justify-center overflow-hidden">
            ...
            <Package className="h-10 w-10 text-stone-900" />
          </div>
        </div>
        <div className="text-center space-y-3">
           <h2 className="text-[14px] font-black text-stone-900 uppercase tracking-[0.4em] pl-[0.4em]">Affiliate Marketplace</h2>
      ...
      <div className="max-w-[1440px] mx-auto space-y-12 px-8 pt-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
           <div className="flex items-center gap-6">
              <Button variant="ghost" size="icon" asChild className="shrink-0 rounded-[20px] bg-white border border-stone-100 shadow-sm hover:bg-stone-50 active:scale-95 transition-all text-stone-600 h-14 w-14">
                <Link href="/dashboard/affiliate/analytics"><ArrowLeft className="h-6 w-6" /></Link>
              </Button>
              <div className="space-y-2">
                 <h1 className="text-4xl font-black text-stone-900 tracking-tighter">Promoted Products</h1>
        ...
        <GlassCard className="rounded-[48px] border-white bg-white/60 shadow-xl overflow-hidden">
           <div className="p-10 border-b border-stone-100 flex items-center justify-between">
              <div className="space-y-1">
                 <h3 className="text-3xl font-black text-stone-900 tracking-tighter">Inventory Performance</h3>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-stone-50/40">
                    <th className="py-8 pl-10 pr-6 text-[11px] font-black uppercase tracking-[0.3em] text-stone-400 border-b border-stone-100">Product / Link</th>
                    <th className="py-8 px-6 text-center text-[11px] font-black uppercase tracking-[0.3em] text-stone-400 border-b border-stone-100">Commission</th>
                    <th className="py-8 px-6 text-right text-[11px) font-black uppercase tracking-[0.3em] text-stone-400 border-b border-stone-100">Clicks</th>
                    <th className="py-8 px-6 text-right text-[11px] font-black uppercase tracking-[0.3em] text-stone-400 border-b border-stone-100">Sales</th>
                    <th className="py-8 px-6 text-right text-[11px] font-black uppercase tracking-[0.3em] text-emerald-500 border-b border-stone-100">Earnings</th>
                  ...
                  {rows.length === 0 ? (
                    <tr>
                       <td colSpan={6} className="py-24 text-center space-y-6">
                          <div className="w-24 h-24 bg-white rounded-[40px] flex items-center justify-center mx-auto mb-6 border border-white shadow-xl">
                             <Package className="h-10 w-10 text-stone-100" />
                          </div>
                          <p className="text-2xl font-black text-stone-900 tracking-tighter">No Promoted Items Yet</p>
                    ...
                    rows.map((r) => {
                      ...
                      return (
                        <tr key={r.id as string} className="hover:bg-white/80 transition-all duration-500 group">
                          <td className="py-10 pl-10 pr-6">
                            <div className="flex items-center gap-6">
                              <div className="w-20 h-20 shrink-0 rounded-[28px] bg-white border border-white shadow-xl overflow-hidden flex items-center justify-center transition-transform group-hover:scale-110 duration-700">
                                {imgSrc ? (
                                ...
                                {hasProduct ? (
                                  <>
                                    <Link
                                      ...
                                      className="font-black text-xl text-stone-900 hover:text-orange-500 transition-colors truncate block tracking-tighter"
                                    >
                                      {product?.name}
                                    </Link>
                          ...
                          <td className="py-10 px-6 text-center">
                            <div className="space-y-1">
                               <GlassPill color="orange" className="font-black text-[11px] px-4 py-2 border-white shadow-sm">{rate}% Share</GlassPill>
                          ...
                          <td className="py-10 px-6 text-right">
                             <p className="text-2xl font-black text-stone-900 tabular-nums tracking-tighter">{(r.total_clicks as number) ?? 0}</p>
                             <p className="text-[9px] font-black text-stone-300 uppercase tracking-widest mt-1">Total Clicks</p>
                          </td>
                          <td className="py-10 px-6 text-right">
                             <p className="text-2xl font-black text-stone-900 tabular-nums tracking-tighter">{(r.total_conversions as number) ?? 0}</p>
                          ...
                          <td className="py-10 pr-10 text-right">
                            <Button variant="ghost" size="icon" className="h-12 w-12 rounded-2xl bg-white border border-transparent shadow-lg text-stone-400 hover:bg-stone-900 hover:text-white transition-all active:scale-90" asChild>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-8">
           <GlassCard className="p-10 rounded-[48px] bg-white/60 border-white shadow-xl flex flex-col justify-between">
              <TrendingUp className="h-10 w-10 text-sky-500 mb-8" />
              <div>
                 <h4 className="text-xl font-black text-stone-900 tracking-tighter">Sales Velocity</h4>
           ...
           <GlassCard className="p-10 rounded-[48px] bg-white/60 border-white shadow-xl flex flex-col justify-between">
              ...
              <div>
                 <h4 className="text-xl font-black text-stone-900 tracking-tighter">Earn More</h4>
           ...
           <GlassCard className="p-10 rounded-[48px] bg-stone-900 text-white shadow-2xl flex flex-col justify-between border-none">
