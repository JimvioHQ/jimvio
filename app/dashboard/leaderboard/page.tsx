    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center space-y-12 animate-in fade-in duration-700" style={{ background: "#f0ede8" }}>
        <div className="relative">
          <div className="absolute inset-0 bg-amber-400/20 blur-3xl rounded-sm scale-150 animate-pulse" />
          <div className="relative w-24 h-24 rounded-sm bg-white dark:bg-surface border border-white shadow-none flex items-center justify-center overflow-hidden">
            <div className="absolute inset-0 border-2 border-t-amber-500 border-r-transparent border-b-transparent border-l-transparent rounded-sm animate-spin m-2" />
    ...
    >
      ...
      <div className="max-w-[1440px] mx-auto space-y-12 px-8 pt-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
           ...
           <div className="bg-amber-100/40 backdrop-blur-xl text-amber-700 px-8 py-3.5 rounded-sm text-[10px] font-black uppercase tracking-widest flex items-center gap-3 border border-amber-200/50 shadow-none animate-pulse">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
           <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
             {leaders.slice(0,3).map((l, i) => (
                <GlassCard key={l.id} className={cn(
                  "relative overflow-hidden group transition-all duration-700 hover:-translate-y-4 rounded-sm",
                   ...
                   <div className="p-10 text-center h-full flex flex-col justify-between relative z-10">
                      <div className="space-y-6">
                         <div className="relative inline-block mx-auto mt-4">
                            <Avatar className="h-32 w-32 border-8 border-white shadow-none rounded-sm">
                              <AvatarFallback className={cn("text-4xl font-black text-white rounded-sm", i === 0 ? "bg-amber-500 shadow-[0_0_30px_rgba(245,158,11,0.5)]" : i === 1 ? "bg-stone-400" : "bg-orange-500")}>
                                {l.affiliate_code?.slice(0,2).toUpperCase()}
                              </AvatarFallback>
                            </Avatar>
                            <div className={cn(
                              "absolute -bottom-4 -right-4 w-12 h-12 rounded-sm border-4 border-white flex items-center justify-center font-black text-white text-[16px] shadow-none",
           ...
           {/* Network Tier Hub */}
           <GlassCard className="lg:col-span-4 rounded-sm bg-stone-900 text-white flex flex-col justify-between relative overflow-hidden shadow-none border-none">
              <div className="absolute top-0 right-0 w-80 h-80 bg-emerald-500/20 blur-[120px] rounded-sm translate-x-1/2 -translate-y-1/2 pointer-events-none" />
              <div className="absolute bottom-0 left-0 w-80 h-80 bg-orange-500/20 blur-[120px] rounded-sm -translate-x-1/2 translate-y-1/2 pointer-events-none" />
              <div className="p-12 space-y-10 relative z-10">
                 ...
                 <Button className="w-full bg-white dark:bg-surface text-stone-900 dark:text-white hover:bg-stone-50 dark:bg-surface/50 font-black text-[11px] uppercase tracking-widest h-16 rounded-sm shadow-none active:scale-95 transition-all mt-4 border-none">
        ...
        <GlassCard className="rounded-sm border-white bg-white dark:bg-surface/60 shadow-none overflow-hidden">
           <div className="p-10 border-b border-stone-100 dark:border-border flex flex-col md:flex-row items-center justify-between gap-6">
              ...
              <div className="bg-white dark:bg-surface/60 p-2 rounded-sm border border-white shadow-none flex items-center gap-2">
                 <div className="px-6 py-2 rounded-sm bg-stone-900 text-white text-[10px] font-black uppercase tracking-widest shadow-none">Volume Flow</div>
                 <div className="px-6 py-2 rounded-sm text-stone-400 text-[10px] font-black uppercase tracking-widest hover:text-stone-900 dark:text-white transition-colors cursor-pointer">Acquisition Yield</div>
              </div>
           </div>
                  ...
                  {leaders.map((l, i) => (
                    <tr key={l.id} className="hover:bg-white dark:bg-surface/80 transition-all duration-500 group">
                      <td className="py-10 pl-10">
                         <div className={cn(
                           "flex items-center justify-center w-12 h-12 rounded-sm text-[14px] font-black border shadow-none transition-transform group-hover:scale-110",
                      ...
                      <td className="py-10 px-6">
                        <div className="flex items-center gap-6">
                          <Avatar className="h-14 w-14 border border-white shadow-none rounded-sm group-hover:scale-105 transition-transform duration-500">
                            <AvatarFallback className="text-[14px] font-black bg-stone-50 dark:bg-surface/50 text-stone-900 dark:text-white rounded-sm">@{l.affiliate_code?.[0]?.toUpperCase()}</AvatarFallback>
                          </Avatar>
                          <div>
                      ...
                      <td className="py-10 pr-10 text-right">
                         <Button variant="ghost" size="icon" className="h-12 w-12 rounded-sm bg-white dark:bg-surface border border-transparent shadow-none text-stone-300 group-hover:bg-stone-900 group-hover:text-white transition-all active:scale-90">
