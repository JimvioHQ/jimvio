      ...
      <div className="min-h-screen flex flex-col items-center justify-center space-y-12 animate-in fade-in duration-700" style={{ background: "#f0ede8" }}>
        <div className="relative">
          ...
          <div className="relative w-24 h-24 rounded-[32px] bg-white border border-white shadow-2xl flex items-center justify-center overflow-hidden">
            ...
            <Trophy className="h-10 w-10 text-stone-900" />
          </div>
        </div>
        <div className="text-center space-y-3">
           <h2 className="text-[14px] font-black text-stone-900 uppercase tracking-[0.4em] pl-[0.4em]">Propagator Census</h2>
    ...
    >
      ...
      <div className="max-w-[1440px] mx-auto space-y-12 px-8 pt-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
           ...
           <div className="bg-amber-100/40 backdrop-blur-xl text-amber-700 px-8 py-3.5 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-3 border border-amber-200/50 shadow-xl animate-pulse">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
           <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
             {leaders.slice(0,3).map((l, i) => (
                <GlassCard key={l.id} className={cn(
                  ...
                  i === 1 ? "h-[440px] border-stone-200 bg-white/60 order-2 md:order-1" : 
                  "h-[400px] border-orange-200 bg-white/60 order-3 md:order-3"
                )}>
                   ...
                   <div className="p-10 text-center h-full flex flex-col justify-between relative z-10">
                      <div className="space-y-6">
                         <div className="relative inline-block mx-auto mt-4">
                            <Avatar className="h-32 w-32 border-8 border-white shadow-2xl rounded-[40px]">
                              <AvatarFallback className={cn("text-4xl font-black text-white rounded-[32px]", i === 0 ? "bg-amber-500 shadow-[0_0_30px_rgba(245,158,11,0.5)]" : i === 1 ? "bg-stone-400" : "bg-orange-500")}>
                                {l.affiliate_code?.slice(0,2).toUpperCase()}
                              </AvatarFallback>
                            </Avatar>
                            <div className={cn(
                              "absolute -bottom-4 -right-4 w-12 h-12 rounded-[18px] border-4 border-white flex items-center justify-center font-black text-white text-[16px] shadow-2xl",
                         ...
                         <div>
                           <p className="font-black text-2xl text-stone-900 tracking-tighter">@{l.affiliate_code}</p>
                           <p className="text-[10px] font-black uppercase text-stone-400 tracking-[0.3em] mt-2">{l.total_conversions || 0} Acquisitions</p>
                         </div>
                      </div>
                      <div className="pt-8">
                        <p className="text-3xl font-black text-stone-900 tracking-tighter drop-shadow-sm">{formatMoney(Number(l.total_earnings), "USD")}</p>
                        <GlassPill color={i === 0 ? "amber" : "default"} className="mt-6 text-[10px] font-black py-2 shadow-sm border-white">
           ...
           {/* Network Tier Hub */}
           <GlassCard className="lg:col-span-4 rounded-[48px] bg-stone-900 text-white flex flex-col justify-between relative overflow-hidden shadow-2xl border-none">
              <div className="absolute top-0 right-0 w-80 h-80 bg-emerald-500/20 blur-[120px] rounded-full translate-x-1/2 -translate-y-1/2 pointer-events-none" />
              <div className="absolute bottom-0 left-0 w-80 h-80 bg-orange-500/20 blur-[120px] rounded-full -translate-x-1/2 translate-y-1/2 pointer-events-none" />
              <div className="p-12 space-y-10 relative z-10">
                 ...
                 <Button className="w-full bg-white text-stone-900 hover:bg-stone-50 font-black text-[11px] uppercase tracking-widest h-16 rounded-[28px] shadow-2xl active:scale-95 transition-all mt-4 border-none">
        ...
        <GlassCard className="rounded-[48px] border-white bg-white/60 shadow-xl overflow-hidden">
           <div className="p-10 border-b border-stone-100 flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="space-y-1">
                 <h3 className="text-3xl font-black text-stone-900 tracking-tighter">Global Rankings</h3>
                 <p className="text-[11px] font-black uppercase tracking-widest text-stone-400">Total trade flow & node performance registry</p>
              </div>
              <div className="bg-white/60 p-2 rounded-full border border-white shadow-xl flex items-center gap-2">
                 ...
                 <div className="px-6 py-2 rounded-full text-stone-400 text-[10px] font-black uppercase tracking-widest hover:text-stone-900 transition-colors cursor-pointer">Acquisition Yield</div>
              </div>
           </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-stone-50/40">
                    <th className="py-8 pl-10 w-24 text-[11px] font-black uppercase tracking-[0.3em] text-stone-400 border-b border-stone-100">Pos</th>
                    <th className="py-8 px-6 text-[11px] font-black uppercase tracking-[0.3em] text-stone-400 border-b border-stone-100">Trade Proxy</th>
                    <th className="py-8 px-6 text-right text-[11px] font-black uppercase tracking-[0.3em] text-stone-400 border-b border-stone-100">Settled Volume</th>
                    <th className="py-8 px-6 text-right text-[11px] font-black uppercase tracking-[0.3em] text-stone-400 border-b border-stone-100">Pulse Count</th>
                    <th className="py-8 px-6 text-right text-[11px] font-black uppercase tracking-[0.3em] text-emerald-500 border-b border-stone-100">Yield Flux</th>
                    <th className="py-8 pr-10 border-b border-stone-100" />
                  </tr>
                </thead>
                <tbody className="divide-y divide-stone-100">
                  {leaders.map((l, i) => (
                    <tr key={l.id} className="hover:bg-white/80 transition-all duration-500 group">
                      <td className="py-10 pl-10">
                         <div className={cn(
                           ...
                           i < 3 ? "bg-amber-100 text-amber-600 border-amber-200/50" : "bg-white text-stone-300 border-white shadow-none"
                         )}>{i+1}</div>
                      </td>
                      <td className="py-10 px-6">
                        <div className="flex items-center gap-6">
                          <Avatar className="h-14 w-14 border border-white shadow-xl rounded-2xl group-hover:scale-105 transition-transform duration-500">
                            <AvatarFallback className="text-[14px] font-black bg-stone-50 text-stone-900 rounded-2xl">@{l.affiliate_code?.[0]?.toUpperCase()}</AvatarFallback>
                          </Avatar>
                          <div>
                            <p className="text-xl font-black text-stone-900 tracking-tighter">@{l.affiliate_code}</p>
                            <p className="text-[10px] text-stone-300 font-black uppercase tracking-widest mt-1">Intelligence Proxy</p>
                          </div>
                        </div>
                      </td>
                      <td className="py-10 px-6 text-right font-black text-2xl text-stone-900 tabular-nums tracking-tighter">{formatMoney(Number(l.total_earnings), "USD")}</td>
                      ...
                      <td className="py-10 pr-10 text-right">
                         <Button variant="ghost" size="icon" className="h-12 w-12 rounded-2xl bg-white border border-transparent shadow-lg text-stone-300 group-hover:bg-stone-900 group-hover:text-white transition-all active:scale-90">
