  ...
  return (
    <div className="min-h-screen pb-24 bg-[#060606] relative">
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-10 px-4 sm:px-6 pt-6 sm:pt-12 relative z-10 animate-in fade-in duration-500">
        ...
        {/* Header Protocol */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-[#222]">
           <div className="flex items-center gap-4">
              <div className="p-3 bg-[#111] border border-[#222] shrink-0">
                 <Package className="h-6 w-6 text-orange-500" />
              </div>
              <div>
                 <h1 className="text-2xl font-bold text-white tracking-tight">
                    My Products
                    <span className="text-zinc-600 ml-3 font-mono text-lg">({products.length})</span>
                 </h1>
                 <p className="text-[11px] font-bold text-zinc-500 uppercase tracking-widest mt-1 font-mono">
           ...
           <div className="flex items-center gap-4">
              <Button className="h-10 px-8 rounded-none bg-orange-600 text-white font-bold text-xs uppercase tracking-widest shadow-[0_0_20px_rgba(249,115,22,0.2)] hover:bg-orange-500 transition-all border-none" asChild>
        ...
        {/* Stats Row */}
         <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
           <div className="p-6 flex flex-col justify-between bg-[#0A0A0A] border border-[#222] group">
              <div className="w-12 h-12 bg-[#111] flex items-center justify-center mb-6 group-hover:bg-[#1A1A1A] transition-colors">
                 <Store className="h-5 w-5 text-orange-500" />
              </div>
              <div>
                 <p className="text-3xl font-black text-white tracking-tight tabular-nums">{products.filter(p => p.status === "active").length}</p>
                 <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mt-2">Active Items</p>
              </div>
           </div>
           ...
           <div className="p-6 flex flex-col justify-between bg-[#0A0A0A] border border-[#222] group">
              ...
              <div>
                 <p className="text-3xl font-black text-white tracking-tight tabular-nums">{totalSales}</p>
                 <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mt-2">Total Sales</p>
              </div>
           </div>
           ...
           <div className="p-6 flex flex-col justify-between bg-[#0A0A0A] border border-[#222] group">
              ...
              <div>
                 <p className="text-3xl font-black text-rose-500 tracking-tight tabular-nums">{lowStock}</p>
                 <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mt-2">Low Stock Alerts</p>
              </div>
           </div>
           ...
           <div className="p-6 flex flex-col justify-between bg-[#0A0A0A] border border-[#222] group">
              ...
              <div>
                 <p className="text-3xl font-black text-white tracking-tight tabular-nums">{products.filter(p => p.is_digital).length}</p>
                 <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mt-2">Digital Files</p>
              </div>
           </div>
        </div>
           ...
           <div className="flex flex-col md:flex-row gap-4 sm:gap-6">
              <div className="relative flex-1 group">
                 ...
                 <input
                    ...
                    className="w-full h-12 pl-12 pr-6 bg-[#0A0A0A] border border-[#222] text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:border-zinc-500 transition-colors"
                 />
              </div>
              <div className="flex gap-2 overflow-x-auto no-scrollbar pt-1 shrink-0">
                 {["All", "Active", "Digital", "Physical", "Low Stock"].map((f) => (
                    <button
                       key={f}
                       onClick={() => setFilter(f)}
                       className={cn(
                          "px-6 h-10 text-[10px] font-bold uppercase tracking-widest whitespace-nowrap transition-all border",
                          activeFilter === f
                  ...
                  <div className="grid grid-cols-1 gap-4">
                     {filtered.map((p) => {
                       ...
                       const img = (p.images as string[])?.[0];
                        ...
                        return (
                          <div key={p.id as string} className="group p-5 flex flex-col lg:flex-row lg:items-center gap-6 bg-[#0A0A0A] border border-[#222] hover:border-[#333] transition-colors">
                             
                             {/* Product Graphics */}
                             <div className="w-20 h-20 bg-[#111] border border-[#222] flex items-center justify-center shrink-0 overflow-hidden relative">
                                {img ? (
                                ...
                                {Boolean(p.is_digital) && (
                                   <div className="absolute top-0 right-0 bg-orange-600 text-white text-[8px] font-bold px-1.5 py-0.5 border-b border-l border-[#222]">
                                      DIGITAL
                                   </div>
                                )}
                             </div>
                             ...
                             <div className="flex-1 min-w-0 space-y-2">
                                <div className="flex items-center gap-3">
                                   <button onClick={() => toggleStatus(p.id as string, p.status as string)} className="active:scale-95 transition-all">
                                      <span className={cn("text-[9px] font-bold uppercase tracking-widest px-2.5 py-1 flex items-center justify-center border", s.bgClass, s.textClass)}>
                                        {s.label}
                                      </span>
                                   </button>
                                   <span className="text-[10px] font-mono text-zinc-600 uppercase">#{String(p.id).slice(0, 8)}</span>
                                </div>
                                <h3 className="text-xl font-bold text-white tracking-tight truncate leading-tight group-hover:text-orange-500 transition-colors">{p.name as string}</h3>
                                
                                <div className="flex flex-wrap items-center gap-8 pt-2">
                                   <div className="space-y-1">
                                      <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest">Price</p>
                                      <span className="text-base font-bold text-white font-mono">{formatMoney(Number(p.price), (p.currency as string) || undefined)}</span>
                                   </div>
                                   <div className="space-y-1">
                                      <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest">Sales</p>
                                      <div className="flex items-center gap-1.5">
                                         <TrendingUp className="h-3.5 w-3.5 text-zinc-500" />
                                         <span className="text-base font-bold text-white font-mono">{(p.sale_count as number) || 0}</span>
                                      </div>
                                   </div>
                                   {!p.is_digital && (
                                      <div className="space-y-1">
                                         <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest">Stock</p>
                                         <div className="flex items-center gap-2">
                                            <div className={cn("w-1.5 h-1.5", (p.inventory_quantity as number) <= 5 ? "bg-rose-500 animate-pulse" : "bg-zinc-600")} />
                                            <span className={cn(
                             ...
                             <div className="flex items-center gap-2 shrink-0 w-full lg:w-auto">
                                <Link href={`/dashboard/products/${p.id as string}/edit`} className="flex-1 lg:flex-none">
                                   <div className="h-10 px-5 bg-[#111] border border-[#222] hover:bg-[#1A1A1A] flex items-center justify-center gap-2 text-zinc-400 hover:text-white transition-colors cursor-pointer active:scale-95">
                                 ...
                                 <button onClick={() => deleteProduct(p.id as string)} className="w-10 h-10 bg-[#111] border border-[#222] hover:border-rose-500/50 hover:bg-rose-500/10 flex items-center justify-center text-zinc-500 hover:text-rose-500 transition-colors cursor-pointer active:scale-95">
