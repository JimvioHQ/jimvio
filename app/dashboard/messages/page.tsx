...
export default function MessagesPage() {
  ...
  // FIX: Stable supabase client — createClient() must NOT be called on every render
  ...
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [messages, setMessages] = useState<MessageRow[]>([]);
  // FIX: Separate loading states so loading=false only after everything is done
  ...
  // FIX: Track whether URL params have been processed to avoid repeat inserts
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      // Load vendor options (for starting new conversations)
        ...
        .then(({ data }) => {
          ...
          (data ?? []).forEach((o: any) => {
            ...
            if (vv?.id && !seen.has(vv.id)) {
              seen.add(vv.id);
              vendors.push({ id: vv.id, business_name: vv.business_name ?? "Store", business_slug: vv.business_slug ?? "" });
            }
          });
          setVendorOptions(vendors);
        });

      // Auth is done, conversations loading begins
      setAuthLoading(false);
      await loadConversations(user.id, vid);
    });
  }, [supabase, loadConversations]);

  /* ── Last message previews — only re-fetch when conv list IDs actually change ── */
  // FIX: stable string key, not inline .join() in deps
  const convIdKey = useMemo(
    () => conversations.map(c => c.id).sort().join(","),
    [conversations.map(c => c.id).join(",")]  // eslint-disable-line
  );
        ...
        (data ?? []).forEach((m: any) => {
          if (!byConv[m.conversation_id]) {
            byConv[m.conversation_id] = {
              body: m.body ?? "", message_type: m.message_type ?? "text", created_at: m.created_at,
            };
          }
        });
  ...
  useEffect(() => {
    // FIX: only run once after loading finishes and only if not already handled
    ...
    if (vendorParam) {
      // Check in already-loaded list first
      ...
      // Check DB for existing conversation
      supabase.from("conversations")
        .select("id").eq("buyer_id", userId).eq("vendor_id", vendorParam).maybeSingle()
        .then(({ data: found }) => {
          if (found) {
            ...
            setConversations(prev => {
              ...
              return [{
                ...
                updated_at: new Date().toISOString(), vendors: v ?? null,
  ...
  }, [userId, loading]); // FIX: intentionally minimal deps — runs once after load
  ...
  useEffect(() => {
    ...
    const sub = supabase
      ...
      .on(
        ...
        (payload) => {
          setMessages(prev => {
            // FIX: deduplicate — real-time may fire alongside optimistic insert
    ...
    return () => { supabase.removeChannel(sub); }; // FIX: use removeChannel, not sub.unsubscribe()
  }, [selectedId, userId, supabase]);
  ...
  /* ── Insert message (text, quote, product share) ── */
  async function insertMessage(payload: {
    ...
    // Update last_message preview locally (real-time handles the message itself)
    ...
    setShowProductShare(false);
    setSendingQuote(false);
    // FIX: clear replyTo after sending
    setReplyTo(null);
  }
  ...
  // FIX: memoize so it's not rebuilt on every render
                ...
                filteredConvs.map(c => (
                  <ConvRow
                    ...
                    onClick={() => {
                      setSelectedId(c.id);
                      setMobileShowChat(true);
                      // FIX: clear reply when switching conversations
                      setReplyTo(null);
                    }}
                  />
                ))
              ...
              <>
                    ...
                    messages.map(m => (
                      <div key={m.id}>
                        <MessageBubble
                          ...
                          onReply={setReplyTo}
                        />
                        {/* FIX: vendor "Create offer" button only for unanswered quote requests */}
                        {isVendor
                          && m.sender_id !== userId
                ...
                <div className="px-4 pb-4 pt-2 border-t border-border flex-shrink-0">
                  <ChatInput
                    ...
                    userId={userId!}
                    onSent={() => {
                      // FIX: refresh last message preview after ChatInput sends
                      setConversations(prev =>
                        prev.map(c =>
                          c.id === selectedId
                    ...
                    // FIX: pass full replyTo object so ChatInput can show preview + attach reply_to_id
                    replyTo={replyTo ? { id: replyTo.id } : null}
                    onCancelReply={() => setReplyTo(null)}
                    isVendor={isVendor}
                    onRequestQuote={!isVendor ? () => setShowQuoteRequest(true) : undefined}
        ...
        <QuoteRequestModal
          ...
          onSubmit={async (data) => {
            ...
            await insertMessage({
              body: data.body ?? "",
              message_type: "quote_request",
              // FIX: include reply_to_id if replying
        ...
        <QuoteReplyModal
          ...
          onSubmit={async (data) => {
            ...
            await insertMessage({
              body: data.body ?? "",
              message_type: "quote_reply",
              // FIX: link reply back to the original quote_request message
