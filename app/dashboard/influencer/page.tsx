   ...
   useEffect(() => {
      async function load() {
         ...
         if (inf) {
            ...
            const totalViews = (submissionsList as { view_count: number | null }[])
               .reduce((s: number, c) => s + (Number(c.view_count) || 0), 0);
            const campaignIds = new Set((submissionsList as any[]).map(c => c.ugc_campaigns?.id).filter(Boolean));
            setStats({
