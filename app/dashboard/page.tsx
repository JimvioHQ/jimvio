...
// import type { TrendingCampaignItem, RecommendedCreator, RecentSearchItem } from "@/components/dashboard/DashboardSidebar";
...
// function classifyTx(type: string): "affiliate" | "ugc" | "marketplace" | "communities" {
...
// function buildSparkline(txs: TxRow[], bucket: "affiliate" | "ugc" | "marketplace" | "communities", days = 8): number[] {
//   const end = new Date();
...
//   return (
//     <div className={`animate-pulse rounded-xl bg-surface-secondary ${className}`} />
//   );
// }
...
//     const [
//       profileRes, walletRes, txRes, txTodayRes,
//       txYesterdayRes,
//       notifsRes, campaignsRes, productsRes, membershipsRes,
//       affiliateRes, influencerRes,
//       trendingCampsRes, creatorsRes, searchesRes,
//     ] = await Promise.all([
...
//       supabase.from("transactions").select("amount, type, direction, created_at").eq("user_id", userId).gte("created_at", since14).order("created_at", { ascending: true }),
//       supabase.from("transactions").select("amount, direction").eq("user_id", userId).gte("created_at", todayStart).eq("direction", "credit"),
//       supabase.from("transactions").select("amount, direction").eq("user_id", userId).gte("created_at", yesterdayStart).lt("created_at", todayStart).eq("direction", "credit"),
//       supabase.from("notifications").select("id, type, title, message, is_read, created_at, action_url").eq("user_id", userId).order("created_at", { ascending: false }).limit(6),
//       supabase.from("ugc_campaigns")
//         .select(`
//           id, title, description, campaign_type, rate_per_1k_views, fixed_rate,
//           payment_model, submission_count, total_budget, ends_at, target_platforms,
//           min_followers,
...
//       // Sidebar: top 3 trending campaigns by submission_count
//       supabase.from("ugc_campaigns").select("id, title, fixed_rate, rate_per_1k_views, payment_model, submission_count").eq("status", "active").order("submission_count", { ascending: false }).limit(3),
//       // Sidebar: top creators (affiliates with most links)
//       supabase.from("profiles").select("id, full_name, username, avatar_url").neq("id", userId).limit(3),
//       // Sidebar: user's recent search history
//       supabase.from("search_history").select("query, created_at").eq("user_id", userId).order("created_at", { ascending: false }).limit(5),
//     ]);
...
//     // Build real earnings chart data
//     const days = 14;
//     const chartEnd = new Date();
//     const chartStart = subDays(chartEnd, days - 1);
//     const range = eachDayOfInterval({ start: startOfDay(chartStart), end: startOfDay(chartEnd) });
...
//     for (const label of labels) buckets[label] = { affiliate: 0, ugc: 0, marketplace: 0, communities: 0 };
//     for (const tx of txs) {
//       if (tx.direction !== "credit") continue;
//       const key = format(new Date(tx.created_at), "MMM d");
...
//     const CHART_COLORS = { affiliate: "#fd5000", ugc: "#8b5cf6", marketplace: "#3b82f6", communities: "#30a46c" };
//     const CHART_LABELS_MAP: Record<string, string> = { affiliate: "Affiliate", ugc: "UGC", marketplace: "Marketplace", communities: "Communities" };
//     const realDatasets: EarningsDataset[] = (["affiliate", "ugc", "marketplace", "communities"] as const).map((bucket) => {
...
//       };
//     });
//     const realTotal = realDatasets.reduce((s, d) => s + d.data.reduce((a, b) => a + b, 0), 0);

//     // Earned today
//     const todayEarned = (txTodayRes.data ?? []).reduce((s: number, r: { amount: number; direction: string }) => s + Number(r.amount), 0);

//     // Real trend calculations
//     const yesterdayEarned = (txYesterdayRes.data ?? []).reduce((s: number, r: { amount: number; direction: string }) => s + Number(r.amount), 0);
//     const earnPctRaw = yesterdayEarned > 0
...
//     // Campaigns trend: show count vs "accepted"
//     const realCampaignsTrend = campCount > 0 ? `${campCount} campaigns joined` : "No campaigns yet";

//     // Links trend: show active count
//     const realLinksTrend = linksCount > 0 ? `${linksCount} active links` : "No active links";

//     // Stat cards with real sparklines
//     const spark = (b: "affiliate" | "ugc" | "marketplace" | "communities") => buildSparkline(txs, b);
...
//         change: w?.pending_balance ? `${formatMoney(w.pending_balance, "USD")} pending` : "No pending",
//         up: true,
//         chartColor: "#fd5000",
...
//     // Activity items from notifications
//     const activity = (notifsRes.data ?? []).map((n) => notifToActivity(n as NotifRow));
...
//       const earnStr = c.payment_model === "per_views"
//         ? `${formatMoney(c.rate_per_1k_views, "USD")}/1k`
//         : formatMoney(c.fixed_rate, "USD");
//       const budgetPct = c.total_budget > 0
//         ? Math.min(100, Math.round((c.submission_count / (c.total_budget / 50)) * 100))
//         : 0;
...
//         name: p.name,
//         category: (p.product_categories as { name: string } | null)?.name ?? "General",
//         price: formatMoney(displayPrice, p.currency ?? "USD"),
//         commission: commission > 0 ? `${commission}% Commission` : "No commission",
...
//     ];
//     const mappedSidebarCampaigns: TrendingCampaignItem[] = ((trendingCampsRes.data ?? []) as { id: string; title: string; fixed_rate: number; rate_per_1k_views: number; payment_model: string; submission_count: number }[]).map((c, i) => ({
//       name: c.title,
...
//     const mappedCreators: RecommendedCreator[] = ((creatorsRes.data ?? []) as { id: string; full_name: string | null; username: string | null; avatar_url: string | null }[]).map((p, i) => {
...
//     const mappedSearches: RecentSearchItem[] = ((searchesRes.data ?? []) as { query: string }[]).map((s) => ({ text: s.query }));
...
//     setWallet(w);
//     setStatCards(cards);
//     setActivityItems(activity.length > 0 ? activity : []);
...
//     { initials: "JC", color: "#fd5000" },
//     { initials: "MK", color: "#8b5cf6" },
//     { initials: "AS", color: "#3b82f6" },
//     { initials: "RT", color: "#30a46c" },
//   ];
...
//             />
//           ) : (
//             <div className="w-12 h-12 rounded-full flex-shrink-0 flex items-center justify-center text-white font-bold text-lg border-2 border-border" style={{ background: "#fd5000" }}>
...
//                 tagline={`${featuredCampaign.campaign_type.replace("_", " ")} campaign — earn real money creating content.`}
//                 badges={[featuredCampaign.campaign_type.replace("_", " "), "UGC"]}
//                 creatorsJoined={featuredCampaign.submission_count}
//                 spotsLeft={Math.max(0, Math.ceil((featuredCampaign.total_budget / 50) - featuredCampaign.submission_count))}
//                 alreadyPaid={formatMoney(
//                   featuredCampaign.submission_count * (featuredCampaign.fixed_rate || featuredCampaign.rate_per_1k_views || 0),
//                   "USD"
//                 )}
//                 earnUpTo={
...
//                 extraCount={Math.max(0, featuredCampaign.submission_count - 4)}
//                 coverImage={featuredCampaign.ugc_campaign_media?.find((m) => m.type === "image")?.url ?? null}
//                 href={`/dashboard/campaigns/${featuredCampaign.id}`}
//               />
//             ) : (
//               <div className="rounded-2xl border border-dashed border-border bg-surface p-8 text-center text-text-muted text-sm">
//                 No active campaigns right now. <a href="/ugc" className="text-[#fd5000] font-semibold">Browse campaigns →</a>
//               </div>
//             )}
...
export default function DashboardPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    // Activity items
    const activity = (notifsRes.data ?? []).map((n) =>
