      ...
      <Card>
        ...
        <CardContent className="space-y-6 max-w-3xl">
          {(["vendor", "shopify", "cj"] as const).map((key) => (
            <div key={key} className="rounded-sm border border-[var(--color-border)] p-4 grid sm:grid-cols-3 gap-3 items-end">
          ...
          <Button type="button" onClick={saveSources} loading={pendingSources} className="rounded-sm">
      ...
      <Card>
        ...
        <CardContent className="grid sm:grid-cols-2 gap-4 max-w-3xl">
          <div className="grid gap-2">
            ...
            <Input className="rounded-sm" value={social.success_rate_display} onChange={(e) => setSocial((s) => ({ ...s, success_rate_display: e.target.value }))} />
          </div>
          <div className="grid gap-2">
            <Label>Countries label</Label>
            <Input className="rounded-sm" value={social.countries_display} onChange={(e) => setSocial((s) => ({ ...s, countries_display: e.target.value }))} />
          </div>
          <div className="grid gap-2">
            <Label>Fallback vendors label</Label>
            <Input className="rounded-sm" value={social.fallback_verified_vendors} onChange={(e) => setSocial((s) => ({ ...s, fallback_verified_vendors: e.target.value }))} />
          </div>
          <div className="grid gap-2">
            <Label>Fallback products label</Label>
            <Input className="rounded-sm" value={social.fallback_total_products} onChange={(e) => setSocial((s) => ({ ...s, fallback_total_products: e.target.value }))} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-6 max-w-3xl">
          <div className="grid gap-2">
            ...
            <Textarea className="rounded-sm min-h-[90px] font-mono text-sm" value={affiliatePropLines} onChange={(e) => setAffiliatePropLines(e.target.value)} />
          </div>
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-2">
              <Label>Trust bar items</Label>
              <Button type="button" variant="outline" size="sm" className="rounded-sm" onClick={addTrustRow}>
                Add row
              </Button>
            </div>
            <div className="space-y-3">
      ...
      <Card>
        ...
        <CardContent className="grid sm:grid-cols-2 gap-4 max-w-3xl">
          <div className="grid gap-2 sm:col-span-2">
            ...
            <Input className="rounded-sm" value={contact.support_email} onChange={(e) => setContact((c) => ({ ...c, support_email: e.target.value }))} />
          </div>
          <div className="grid gap-2 sm:col-span-2">
            <Label>Info / general email</Label>
            <Input className="rounded-sm" value={contact.info_email} onChange={(e) => setContact((c) => ({ ...c, info_email: e.target.value }))} />
          </div>
          <div className="grid gap-2 sm:col-span-2">
            <Label>X (Twitter) URL</Label>
            <Input className="rounded-sm" value={contact.social_x} onChange={(e) => setContact((c) => ({ ...c, social_x: e.target.value }))} />
          </div>
          <div className="grid gap-2 sm:col-span-2">
            <Label>YouTube URL</Label>
            <Input className="rounded-sm" value={contact.social_youtube} onChange={(e) => setContact((c) => ({ ...c, social_youtube: e.target.value }))} />
          </div>
          <div className="grid gap-2 sm:col-span-2">
            <Label>Instagram URL</Label>
            <Input className="rounded-sm" value={contact.social_instagram} onChange={(e) => setContact((c) => ({ ...c, social_instagram: e.target.value }))} />
          </div>
          <div className="grid gap-2 sm:col-span-2">
            <Label>TikTok URL</Label>
            <Input className="rounded-sm" value={contact.social_tiktok} onChange={(e) => setContact((c) => ({ ...c, social_tiktok: e.target.value }))} />
          </div>
          <div className="grid gap-2">
            <Label>HQ address line 1</Label>
            <Input className="rounded-sm" value={contact.hq_line1} onChange={(e) => setContact((c) => ({ ...c, hq_line1: e.target.value }))} />
          </div>
          <div className="grid gap-2">
            <Label>HQ address line 2</Label>
            <Input className="rounded-sm" value={contact.hq_line2} onChange={(e) => setContact((c) => ({ ...c, hq_line2: e.target.value }))} />
          </div>
        </CardContent>
      </Card>

      <Button className="rounded-sm" size="lg" onClick={submit} disabled={pending}>
