      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <Link
            ...
            className="px-6 py-2.5 rounded-sm bg-stone-900 text-white text-sm font-bold hover:bg-stone-800 transition-all shadow-none shadow-stone-900/10"
          >
            Review Submissions
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-8">
          {[
            ...
            <div key={s.label} className="rounded-sm border border-stone-200 dark:border-border bg-white dark:bg-surface p-6 shadow-none">
        ...
        <div className="flex flex-wrap gap-3 mb-8">
          {['active','draft','paused','completed','cancelled'].map((s) => (
            <div key={s} className="flex-1 min-w-[120px] rounded-sm bg-white dark:bg-surface border border-stone-100 dark:border-border p-4 shadow-none text-center">
        ...
        {/* Campaign Table */}
        <div className="bg-white dark:bg-surface rounded-sm border border-stone-200 dark:border-border shadow-none overflow-hidden">
          ...
          {loading ? (
            <div className="p-8 space-y-4">
              {[1,2,3,4,5].map(i => <div key={i} className="h-12 rounded-sm bg-stone-50 dark:bg-surface/50 animate-pulse" />)}
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-stone-100">
                  {campaigns.map((c) => (
                    <tr key={c.id} className="hover:bg-stone-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className="text-[10px] font-black px-2 py-1 rounded-sm bg-stone-100 text-stone-600 uppercase tracking-tighter">
                          {c.campaign_type}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1">
                           ...
                           <div className="h-1.5 w-full bg-stone-100 rounded-sm overflow-hidden">
                              <div 
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                           <div className={`w-2 h-2 rounded-sm ${STATUS_COLORS[c.status].replace('text-', 'bg-')}`} />
