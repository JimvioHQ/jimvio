...
//     (order.payment_status === "completed" || order.payment_status === "paid") ? "completed" : "polling"
//   );
...
//           // Wait for webhook to sync if it hasn't already
//           await refreshCart();
...
//             <div className="absolute inset-0 rounded-2xl bg-green-100 dark:bg-green-950/30 animate-pulse" />
//             <div className="relative w-20 h-20 rounded-2xl bg-green-50 dark:bg-green-950/20 border border-green-100 dark:border-green-900/30 flex items-center justify-center">
...
//                 <circle cx="20" cy="20" r="2.5" className="orbit-dot" fill="#4ade80" />
//                 <circle cx="20" cy="20" r="2.5" className="orbit-dot orbit-dot-2" fill="#86efac" />
//               </svg>
//             </div>
//           </div>
...
//               We're checking your transaction status...
//             </p>
//           </div>
  ...
  return (
    ...
    >
      ...
      <polyline
        ...
        fill="none" className="check-mark"
      />
...
function DetailRow({
  ...
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-[var(--color-border)] last:border-0">
      ...
      <span className={`text-sm font-semibold ${mono ? "font-mono" : ""} ${accent ? "text-green-600 dark:text-green-500" : "text-[var(--color-text-primary)]"}`}>
        {value}
      </span>
    </div>
  );
...
export function CheckoutSuccessClient({ order }: { order: Order }) {
  ...
  // Flutterwave only: fetch transaction details for display.
  // Non-blocking — success UI renders immediately regardless.
  const [fwTransaction, setFwTransaction] = useState<FlutterwaveTransaction | null>(null);

  useEffect(() => {
    // Strip Flutterwave callback params — user sees /checkout/success only
    window.history.replaceState({}, "", `/checkout/success?order=${order.id}`);

    // Clear the cart — webhook has already confirmed payment
    refreshCart().then(() => window.dispatchEvent(new CustomEvent("cart-updated")));

    // Optionally hydrate Flutterwave transaction details for the detail panel
    ...
    // Check status to trigger finalization if webhook is delayed
    if (order.payment_status === "pending") {
      fetch(`/api/orders/${order.id}/status`)
        .catch(err => console.error("[Flutterwave status check error]", err));
    }
    ...
    // refreshCart is stable; order fields won't change on this page
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const label = resolveOrderLabel(order);
  const method = resolvePaymentMethod(
    order.payment_provider ?? fwTransaction?.payment_type ?? null
  );
  ...
  return (
    <>
      ...
      <div className="max-w-lg mx-auto px-4 pb-12">
        ...
        {/* Flutterwave transaction details (renders once fetched) */}
        {fwTransaction && (
          <div className="mt-6 rounded-xl border border-[var(--color-border)] bg-[var(--color-background-primary)] overflow-hidden anim-2">
            <div className="px-4 py-3 border-b border-[var(--color-border)] flex items-center justify-between">
              <span className="text-xs font-semibold uppercase tracking-wider text-[var(--color-text-secondary)]">
              ...
              {fwTransaction.status && <StatusBadge status={fwTransaction.status} />}
            </div>
            <div className="px-4 divide-y divide-[var(--color-border)]">
              <DetailRow label="Transaction ID" value={String(fwTransaction.id)} mono />
              <DetailRow label="Reference" value={fwTransaction.tx_ref} mono />
              <DetailRow
                label="Amount"
                value={`${fwTransaction.currency} ${Number(fwTransaction.charged_amount ?? fwTransaction.amount).toLocaleString()}`}
                accent
              />
              <DetailRow
                label="Payment method"
              ...
              {fwTransaction.created_at && (
                <DetailRow label="Date" value={new Date(fwTransaction.created_at).toLocaleString()} />
              )}
            </div>
          </div>
        )}
        ...
        <div className="mt-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-background-primary)] overflow-hidden anim-3">
          ...
          <div className="px-4 py-3 border-t border-[var(--color-border)] bg-[var(--color-background-secondary)] flex items-center justify-between">
            <span className="text-sm font-semibold text-[var(--color-text-primary)]">Total paid</span>
            <div className="flex items-center gap-2">
              {method && <Badge variant="outline" className="text-xs">{method}</Badge>}
              <span className="text-base font-bold text-green-600 dark:text-green-500">
                {formatDisplayMoney(Number(order.total_amount), order.currency)}
              </span>
            </div>
