...
export default async function CheckoutPage(props: {
  ...
  // ── Branch B: cart checkout — sanitize, then decide ───────────────────────
    ...
    // Multiple pending orders — let user choose, never combine
    if (actionable.length > 1) {
      return <PendingOrdersSelector orders={actionable} />;
    }

    // One pending order left after cleanup — resume it
    if (actionable.length === 1) {
    ...
    // No pending orders — create fresh ones from cart
    if (finalOrders.length === 0) {
      ...
      // Drop any accidental mixed-type orders
      finalOrders = orders
        .map(normaliseOrder)
        .filter((o) => {
