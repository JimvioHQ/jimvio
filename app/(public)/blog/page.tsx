  ...
  if (posts.length === 0) {
    return (
      <div className="bg-[var(--color-bg)] min-h-screen flex flex-col items-center justify-center px-6 py-24">
        ...
        <Button asChild className="mt-8 rounded-sm font-black">
            ...
            <div className="w-full lg:w-96">
              <div className="relative group">
                ...
                <Input
                  ...
                  className="h-16 pl-14 pr-6 bg-[var(--color-bg)] border-2 rounded-sm focus-visible:ring-[var(--color-accent)] shadow-none"
                />
              </div>
            </div>
          </div>
      ...
      <section className="py-20 max-w-[var(--container-max)] mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20">
          <Link
            ...
            className="relative group cursor-pointer overflow-hidden rounded-sm aspect-[4/3] lg:aspect-auto block"
          >
            <img
              src={featuredImage}
              alt={featured.title}
          ...
          <div className="flex flex-col gap-8">
            {sidePosts.map((post) => (
              <Link key={post.id} href={`/blog/${post.slug}`} className="flex gap-6 group cursor-pointer">
                <div className="w-40 h-40 shrink-0 rounded-sm overflow-hidden bg-subtle relative border border-[var(--color-border)]">
                  <img
                    src={
                      post.image_url ??
                ...
                <div className="flex-1 flex flex-col justify-center">
                  ...
                  <div className="flex items-center gap-4 text-[10px] font-black text-muted-c capitalize tracking-widest">
                    ...
                    <span className="w-1 h-1 bg-muted-c rounded-sm" />
        ...
        <div className="flex flex-wrap gap-3 py-10 border-y border-[var(--color-border)] mb-20 justify-center">
          {categories.map((cat) => (
            <button
              key={cat}
              type="button"
              className="px-6 py-2.5 rounded-sm border border-[var(--color-border)] text-sm font-black text-[var(--color-text-primary)] hover:border-[var(--color-accent)] hover:text-[var(--color-accent)] transition-all"
            >
              {cat}
            </button>
          ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {rest.map((post) => (
              <Link key={post.id} href={`/blog/${post.slug}`} className="group cursor-pointer block">
                <div className="aspect-[16/10] bg-subtle rounded-sm mb-6 overflow-hidden border border-[var(--color-border)] relative">
                  <img
                    src={
                      post.image_url ??
                  ...
                  <div className="absolute top-4 right-4 space-x-2">
                    <Button
                      variant="outline"
                      size="icon"
                      className="h-8 w-8 rounded-sm bg-white dark:bg-surface/50 border-none backdrop-blur-md opacity-0 group-hover:opacity-100 transition-opacity"
                      type="button"
                    >
        ...
        <div className="mt-32 bg-ink-dark rounded-sm p-12 md:p-20 text-center relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-2xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Input
                ...
                className="h-16 bg-white dark:bg-surface/5 border-white/10 text-white placeholder:text-white/20 rounded-sm px-6 text-lg"
              />
              <Button className="h-16 px-10 bg-[var(--color-accent)] hover:bg-[var(--color-accent-hover)] font-black rounded-sm text-lg">
                Subscribe
              </Button>
            </div>
          </div>
