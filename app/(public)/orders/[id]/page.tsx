...
function getDigitalAction(subtype: string | null, url: string | null) {
  switch (subtype) {
    ...
    case "software":
    case "ai-tools":
      return { label: "Access software", icon: <ExternalLink className="h-4 w-4" />, href: url ?? "/dashboard/library" };
    default:
      return { label: "Download file", icon: <Download className="h-4 w-4" />, href: url ?? "/dashboard/library" };
  }
}
