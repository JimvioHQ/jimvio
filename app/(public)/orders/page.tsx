    ...
    return (
      <div className="min-h-screen bg-[var(--color-bg)] pt-28 pb-20">
        <div className="max-w-[900px] mx-auto px-4 space-y-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-24 rounded-sm bg-[var(--color-surface-secondary)] animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg)] pt-28 pb-20">
      <div className="max-w-[900px] mx-auto px-4 sm:px-6">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {TABS.map((t) => (
            <button
              ...
              onClick={() => setTab(t)}
              className={cn(
                "rounded-sm px-4 py-1.5 text-xs font-bold uppercase tracking-wide border transition-colors",
                tab === t
        ...
        {filtered.length === 0 ? (
          <div className="rounded-sm border border-dashed border-[var(--color-border)] bg-[var(--color-surface)] p-12 text-center">
          ...
          <ul className="space-y-4">
            {filtered.map((o) => {
              ...
              return (
                <li
                  key={o.id}
                  className="flex flex-col sm:flex-row sm:items-center gap-4 rounded-sm border border-[var(--color-border)] bg-[var(--color-surface)] p-4 shadow-[var(--shadow-none)]"
                >
                  <div className="flex gap-3 flex-1 min-w-0">
                    <div className="h-14 w-14 rounded-sm border border-[var(--color-border)] bg-[var(--color-surface-secondary)] overflow-hidden flex items-center justify-center shrink-0">
                      {first?.product_image ? (
