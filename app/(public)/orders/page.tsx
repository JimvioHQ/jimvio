        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {TABS.map((t) => (
            <button
              ...
              onClick={() => setTab(t)}
              className={cn(
                "rounded-full px-4 py-1.5 text-xs font-bold uppercase tracking-wide border transition-colors",
                tab === t
          ...
          <ul className="space-y-4">
            {filtered.map((o) => {
              ...
              return (
                <li
                  key={o.id}
                  className="flex flex-col sm:flex-row sm:items-center gap-4 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4 shadow-[var(--shadow-sm)]"
                >
                  <div className="flex gap-3 flex-1 min-w-0">
                    <div className="h-14 w-14 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-secondary)] overflow-hidden flex items-center justify-center shrink-0">
                      {first?.product_image ? (
