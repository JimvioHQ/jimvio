      ...
      <div className="bg-surface/95 dark:bg-zinc-900/95 backdrop-blur-xl border-b border-border sticky top-[var(--navbar-height,64px)] z-30">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-11">
          <nav className="flex items-center gap-1 text-[10px] font-semibold text-zinc-400 dark:text-zinc-600 min-w-0">
            ...
            <ChevronRight className="h-3 w-3 shrink-0" />
            <span className="text-zinc-600 dark:text-zinc-300 font-bold truncate">{product.name}</span>
          </nav>
          <button className="flex items-center gap-1 text-zinc-400 dark:text-zinc-600 hover:text-[var(--color-accent)] transition-colors text-[11px] font-semibold shrink-0 ml-4">
                  ...
                  <div className="hidden md:flex items-center gap-3 p-2.5 rounded-lg border border-border bg-surface dark:bg-zinc-900 mt-auto">
                    <div className="h-8 w-8 rounded-md border border-border overflow-hidden shrink-0 bg-surface-secondary dark:bg-zinc-800 flex items-center justify-center">
                      {vendor.business_logo ? (
                        ...
                        <Globe className="h-4 w-4 text-zinc-300 dark:text-zinc-700 dark:text-zinc-300" />
                      )}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-black text-zinc-800 dark:text-white truncate">{vendor.business_name}</p>
              ...
              <Tabs defaultValue="overview">
                  ...
                  <TabsList className="bg-transparent h-auto p-0 gap-0">
                    {["Overview", "Specifications", "Vendor Info", "Reviews"].map((tab) => (
                      <TabsTrigger
                        ...
                        className="px-3 py-3.5 text-[10px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-600 rounded-none bg-transparent data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:text-[var(--color-accent)] data-[state=active]:border-b-2 data-[state=active]:border-[var(--color-accent)] transition-colors"
                      >
                        {tab}
                      </TabsTrigger>
                    ))}
                ...
                <div className="p-5 sm:p-7">
                  ...
                  <TabsContent value="specifications" className="mt-0 focus-visible:outline-none">
                    <h3 className="text-sm font-black text-zinc-900 dark:text-white mb-4">Specifications</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-10">
                      {[
                        ...
                        <div key={label} className="flex items-center justify-between py-3 border-b border-border">
                          <span className="text-[10px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-600">{label}</span>
                          <span className="text-xs font-semibold text-zinc-700 dark:text-zinc-300">{val}</span>
                        </div>
                      ))}
                    </div>
                  </TabsContent>
                      ...
                      <div className="flex flex-col sm:flex-row gap-5 items-start">
                        <div className="h-16 w-16 rounded-xl border-2 border-border overflow-hidden bg-surface-secondary dark:bg-zinc-800 flex items-center justify-center shrink-0">
                          {vendor.business_logo ? (
                            ...
                            <Globe className="h-7 w-7 text-zinc-300 dark:text-zinc-700 dark:text-zinc-300" />
                          )}
                        </div>
                          ...
                          <div key={i} className="pt-6 first:pt-0">
                            <div className="flex items-start justify-between gap-3 mb-2">
                              <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                  <p className="text-xs font-black text-zinc-800 dark:text-white">{rev.profiles?.full_name ?? "Verified Buyer"}</p>
              ...
              <div className="bg-surface dark:bg-zinc-900 rounded-xl border border-border shadow-sm p-5 space-y-4">
                ...
                <div className="flex items-center gap-3">
                  <div className="h-12 w-12 rounded-lg border-2 border-border overflow-hidden bg-surface-secondary dark:bg-zinc-800 flex items-center justify-center shrink-0">
                    {vendor?.business_logo ? (
                      ...
                      <Globe className="h-5 w-5 text-zinc-300 dark:text-zinc-700 dark:text-zinc-300" />
                    )}
                  </div>
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-1 mt-0.5">
                      ...
                      <span className="text-[10px] font-semibold text-zinc-400 dark:text-zinc-600">Verified Partner</span>
                    </div>
                  </div>
                </div>
                {/* Stats */}
                <div className="grid grid-cols-3 gap-2 bg-surface-secondary dark:bg-zinc-800/50 rounded-lg p-2.5">
                  <div className="text-center">
                    ...
                    <p className="text-[8px] font-black uppercase tracking-wider text-zinc-400 dark:text-zinc-600 mt-0.5">Rating</p>
                  </div>
                  <div className="text-center border-x border-border">
                    ...
                    <p className="text-[8px] font-black uppercase tracking-wider text-zinc-400 dark:text-zinc-600 mt-0.5">Followers</p>
                  </div>
                  <div className="text-center">
                    <div className="text-sm font-black text-zinc-800 dark:text-white">99%</div>
                    <p className="text-[8px] font-black uppercase tracking-wider text-zinc-400 dark:text-zinc-600 mt-0.5">Success</p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between text-[10px]">
                    <span className="text-zinc-400 dark:text-zinc-600">Delivery Rate</span>
                ...
                <div className="space-y-2 pt-1 border-t border-border">
                  ...
                  />
                  <Button variant="ghost" size="sm" className="w-full h-8 font-semibold text-[10px] text-zinc-400 dark:text-zinc-600 hover:text-[var(--color-accent)] uppercase tracking-widest" asChild>
          ...
          <section className="mt-16 pt-10 border-t border-border">
            <div className="flex items-end justify-between mb-6">
              <div>
                ...
                <h2 className="text-xl font-black text-zinc-900 dark:text-white">You Might Also Like</h2>
              </div>
              <Link href="/marketplace" className="text-[10px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-600 hover:text-[var(--color-accent)] transition-colors flex items-center gap-1">
